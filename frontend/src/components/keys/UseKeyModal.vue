      ...
      <template v-else>
          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="min-w-0">
              <h3 class="text-sm font-semibold text-emerald-950">
                {{ t('keys.useKeyModal.ccswitch.title') }}
              </h3>
              <p class="mt-1 text-xs leading-5 text-emerald-900/70">
                {{ t('keys.useKeyModal.ccswitch.description') }}
              </p>
            </div>
            ...
            <div class="flex shrink-0 gap-2">
              <a
                ...
                class="btn btn-secondary justify-center rounded-lg px-3 py-2 text-sm"
              >
                <Icon name="download" size="md" class="mr-2" />
                {{ t('keys.useKeyModal.ccswitch.download') }}
              </a>
              <button
                type="button"
                class="btn btn-primary justify-center rounded-lg px-3 py-2 text-sm"
                @click="emit('import-to-ccswitch')"
              >
                <Icon name="upload" size="md" class="mr-2" />
                {{ t('keys.useKeyModal.ccswitch.import') }}
              </button>
            </div>
          </div>
        </div>
        ...
        >
          <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="rounded-lg bg-gray-900 px-3 py-2 text-left sm:min-w-[13rem]">
              <p class="text-[11px] font-semibold uppercase tracking-wide text-gray-400">
                {{ t('keys.useKeyModal.manual.folderLabel') }}
              </p>
              <code class="mt-1 block break-all font-mono text-sm font-semibold text-white">
                {{ codexConfigDir }}
              </code>
              <p class="mt-1 text-xs leading-5 text-gray-400">
                {{ codexOpenDirectoryHint }}
              </p>
            </div>
          </div>
          ...
          <ol
            ...
            :aria-label="t('keys.useKeyModal.manual.stepsLabel')"
          >
            <li
              v-for="step in manualConfigSteps"
              :key="step.number"
              class="rounded-lg border border-gray-200 bg-gray-50 p-2.5 dark:border-dark-700 dark:bg-dark-900/60"
            >
              <span class="text-xs font-semibold text-primary-600 dark:text-primary-400">{{ step.number }}</span>
              <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-gray-100">{{ step.title }}</p>
              <p class="mt-1 text-xs leading-5 text-gray-600 dark:text-gray-400">{{ step.description }}</p>
            </li>
          </ol>

          <div class="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800">
            {{ t('keys.useKeyModal.manual.warning') }}
          </div>
        </div>
...
const showCcswitchGuide = computed(() =>
  props.platform === 'openai' && (activeClientTab.value === 'codex' || activeClientTab.value === 'codex-ws')
)
const showManualCodexGuide = computed(() => showCcswitchGuide.value)

const codexConfigDir = computed(() =>
  activeTab.value === 'windows' ? '%USERPROFILE%\\.codex' : '~/.codex'
)

const codexOpenDirectoryHint = computed(() =>
  activeTab.value === 'windows'
    ? t('keys.useKeyModal.manual.openDirWindows')
    : t('keys.useKeyModal.manual.openDirUnix')
)

const manualConfigSteps = computed(() => [
  {
    number: '01',
    title: t('keys.useKeyModal.manual.stepConfigTitle'),
    description: t('keys.useKeyModal.manual.stepConfigDescription')
  },
  {
    number: '02',
    title: t('keys.useKeyModal.manual.stepAuthTitle'),
    description: t('keys.useKeyModal.manual.stepAuthDescription')
  },
  {
    number: '03',
    title: t('keys.useKeyModal.manual.stepRestartTitle'),
    description: t('keys.useKeyModal.manual.stepRestartDescription')
  }
])
...
function generateOpenAIFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const configContent = buildCodexConfigToml(baseUrl)
