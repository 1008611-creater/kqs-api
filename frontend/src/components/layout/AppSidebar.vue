        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-if="item.children?.length">
              ...
              <!-- Children -->
              <div v-if="!sidebarCollapsed && isGroupExpanded(item)" class="mb-1 ml-4 border-l border-white/20 pl-2">
                <router-link
...
const flagUserSubscriptions = () => USER_SUBSCRIPTIONS_VISIBLE

const retiredCustomMenuPattern = /捐赠|赞助|打赏|收款|捐钱|充值\s*\/\s*订阅|订阅|donat|donate|donation|sponsor|tip|purchase|subscription/i

function isRetiredCustomMenuItem(item: { id?: string; label?: string; url?: string }): boolean {
  const haystack = [item.id, item.label, item.url].filter(Boolean).join(' ')
  return retiredCustomMenuPattern.test(haystack)
}

// buildSelfNavItems 构造用户自己的导航项（用户端主菜单和管理员的"我的账户"子菜单共享这组声明）。
// withDashboard=true 时包含仪表盘（用户端），false 时不含（管理员的个人区已经有独立仪表盘入口）。
//
// 条目顺序：密钥 → 用量 → 充值/订阅 → 兑换 → 资料。站内支付入口
// 跟随 payment_enabled 开关展示；支付关闭时仍以卡密兑换为主。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/purchase', label: '购买余额', icon: CreditCardIcon, hideInSimpleMode: true },
    { path: '/shop', label: '订阅商店', icon: TicketIcon, featureFlag: flagUserSubscriptions },
...
const customMenuItemsForUser = computed(() => {
  ...
  return items
    ...
    .filter((item) => !isRetiredCustomMenuItem(item))
