    ...
    >
      <div>
        <label class="input-label">
          ...
          <span v-if="!requiresAccountName" class="text-xs font-normal text-gray-400">
            {{ t('common.optional') }}
          </span>
        </label>
        <input
          v-model="form.name"
          type="text"
          :required="requiresAccountName"
          class="input"
          :placeholder="accountCreateGuide.namePlaceholder"
          data-tour="account-form-name"
        />
        <p v-if="!requiresAccountName" class="input-hint">
          {{ t('admin.accounts.createGuide.openaiOauth.nameOptionalHint') }}
        </p>
      </div>
      <div>
      ...
      <div class="account-create-guide">
        <div class="account-create-guide__copy">
          <span class="account-create-guide__eyebrow">
            {{ t('admin.accounts.createGuide.eyebrow') }}
          </span>
          <h4>{{ accountCreateGuide.title }}</h4>
          <p>{{ accountCreateGuide.description }}</p>
        </div>
        <div class="account-create-guide__steps">
          <span>
            <b>01</b>
            {{ t('admin.accounts.createGuide.nameStep') }}
          </span>
          <span>
            <b>02</b>
            {{ accountCreateGuide.typeStep }}
          </span>
          <span>
            <b>03</b>
            {{ accountCreateGuide.nextStep }}
          </span>
        </div>
      </div>
      ...
      <div v-if="form.platform === 'openai'">
        ...
        <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-3" data-tour="account-form-type">
          ...
          <button
            type="button"
            @click="selectOpenAIAPIKeyMode('official')"
            :class="[
              'flex items-center gap-3 rounded-lg border-2 p-3 text-left transition-all',
              accountCategory === 'apikey' && openaiAPIKeyMode === 'official'
                ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20'
                : 'border-gray-200 hover:border-purple-300 dark:border-dark-600 dark:hover:border-purple-700'
            ]"
          >
          ...
          <button
            type="button"
            @click="selectOpenAIAPIKeyMode('relay')"
            :class="[
              'flex items-center gap-3 rounded-lg border-2 p-3 text-left transition-all',
              accountCategory === 'apikey' && openaiAPIKeyMode === 'relay'
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">API 中转站</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">Base URL + Key 接入</span>
            </div>
          </button>
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        <div
          v-if="form.platform === 'openai' && openaiAPIKeyMode === 'relay'"
          class="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4 shadow-sm dark:border-emerald-900/50 dark:bg-emerald-950/20"
        >
          <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p class="mt-1 text-xs text-emerald-700 dark:text-emerald-300">
                选择后会自动填写 Base URL；API Key 仍只保存在账号凭据里。
              </p>
            </div>
            <button
              type="button"
              class="rounded-full border border-emerald-200 bg-white px-3 py-1.5 text-xs font-semibold text-emerald-800 transition hover:border-emerald-300 hover:bg-emerald-100 dark:border-emerald-800 dark:bg-dark-800 dark:text-emerald-200 dark:hover:bg-emerald-900/30"
              @click="selectOpenAIAPIKeyMode('official')"
            >
              切回官方 OpenAI
            </button>
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
            <button
              v-for="preset in openAIRelayPresets"
              :key="preset.baseUrl"
              type="button"
              :class="[
                'rounded-xl border px-3 py-2 text-left text-sm transition active:scale-[0.98]',
                apiKeyBaseUrl === preset.baseUrl
                  ? 'border-emerald-500 bg-white text-emerald-950 shadow-sm dark:border-emerald-500 dark:bg-emerald-950/40 dark:text-emerald-100'
                  : 'border-emerald-100 bg-white/70 text-emerald-800 hover:border-emerald-300 dark:border-emerald-900/50 dark:bg-dark-800/80 dark:text-emerald-200 dark:hover:border-emerald-600'
              ]"
              @click="applyOpenAIRelayPreset(preset)"
            >
              <span class="block font-semibold">{{ preset.name }}</span>
              <span class="mt-0.5 block truncate text-[11px] opacity-70">{{ preset.host }}</span>
            </button>
          </div>
        </div>
        <div
          v-else-if="form.platform === 'openai'"
          class="rounded-2xl border border-purple-100 bg-purple-50/60 p-4 text-xs text-purple-800 dark:border-purple-900/40 dark:bg-purple-950/20 dark:text-purple-200"
        >
          当前为官方 OpenAI API Key。需要接入中转站时，回到上方选择“API 中转站”。
        </div>
        <div>
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
        <div class="mb-3 flex flex-wrap items-start justify-between gap-3">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.supplyProfile.title') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.supplyProfile.description') }}
            </p>
          </div>
          <div class="flex flex-wrap gap-2">
            <button
              v-for="preset in supplyProfilePresets"
              :key="preset.key"
              type="button"
              class="rounded-md border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-700 transition hover:border-primary-400 hover:text-primary-700 dark:border-dark-600 dark:text-gray-300 dark:hover:border-primary-500 dark:hover:text-primary-300"
              :data-testid="`account-supply-preset-${preset.key}`"
              @click="applySupplyProfilePreset(preset.key)"
            >
              {{ preset.label }}
            </button>
          </div>
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-5">
          ...
          <div>
            ...
            <input
              ...
              id="create-account-rate-multiplier"
              type="number"
              min="0"
              step="0.001"
              class="input"
          ...
          <div>
            <label class="input-label">{{ t('admin.accounts.supplyProfile.maxBodyMB') }}</label>
            <input
              v-model.number="maxRequestBodyMB"
              id="create-account-max-body"
              type="number"
              min="0"
              step="1"
              class="input"
              :placeholder="t('admin.accounts.supplyProfile.maxBodyPlaceholder')"
              @input="normalizeMaxRequestBodyMB"
            />
            <p class="input-hint">{{ t('admin.accounts.supplyProfile.maxBodyHint') }}</p>
          </div>
        </div>
      </div>
      <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
...
const DEFAULT_OPENAI_ALLOWED_MODELS = ['gpt-5.5']
const getDefaultAllowedModels = (platform: string) => (
  platform === 'openai'
...
const openaiAPIKeyMode = ref<'official' | 'relay'>('official')
...
const openAIRelayPresets = [
  { name: '本站网关', host: 'api.cauai.fun', baseUrl: 'https://api.cauai.fun' },
  { name: 'FreeModel', host: 'api.freemodel.dev', baseUrl: 'https://api.freemodel.dev' },
  { name: 'OpenRouter', host: 'openrouter.ai', baseUrl: 'https://openrouter.ai/api/v1' },
  { name: 'AIHubMix', host: 'aihubmix.com', baseUrl: 'https://aihubmix.com/v1' },
  { name: 'DeepSeek', host: 'api.deepseek.com', baseUrl: 'https://api.deepseek.com' },
  { name: 'Moonshot', host: 'api.moonshot.cn', baseUrl: 'https://api.moonshot.cn/v1' },
  { name: 'SiliconFlow', host: 'api.siliconflow.cn', baseUrl: 'https://api.siliconflow.cn/v1' },
  { name: '自定义', host: '手动填写 Base URL', baseUrl: '' }
]

const selectOpenAIAPIKeyMode = (mode: 'official' | 'relay') => {
  accountCategory.value = 'apikey'
  openaiAPIKeyMode.value = mode
  if (mode === 'official') {
    apiKeyBaseUrl.value = 'https://api.openai.com'
    return
  }
  if (!apiKeyBaseUrl.value.trim() || apiKeyBaseUrl.value.trim() === 'https://api.openai.com') {
    apiKeyBaseUrl.value = 'https://api.cauai.fun'
  }
}

const applyOpenAIRelayPreset = (preset: { name: string; baseUrl: string }) => {
  if (!preset.baseUrl) {
    apiKeyBaseUrl.value = ''
    return
  }
...
type SupplyProfilePresetKey = 'lowCostSmall' | 'balanced' | 'fallback'

const maxRequestBodyMB = ref<number | null>(null)

const supplyProfilePresets = computed<Array<{ key: SupplyProfilePresetKey; label: string }>>(() => [
  { key: 'lowCostSmall', label: t('admin.accounts.supplyProfile.presets.lowCostSmall') },
  { key: 'balanced', label: t('admin.accounts.supplyProfile.presets.balanced') },
  { key: 'fallback', label: t('admin.accounts.supplyProfile.presets.fallback') }
])

const normalizeMaxRequestBodyMB = () => {
  const value = Number(maxRequestBodyMB.value)
  if (!Number.isFinite(value) || value <= 0) {
    maxRequestBodyMB.value = null
    return
  }
  maxRequestBodyMB.value = Math.trunc(value)
}

const applySupplyProfilePreset = (preset: SupplyProfilePresetKey) => {
  switch (preset) {
    case 'lowCostSmall':
      form.priority = 1
      form.load_factor = 1
      maxRequestBodyMB.value = 10
      break
    case 'balanced':
      form.priority = 5
      form.load_factor = 2
...
const applyCommonSupplyExtra = (
  base?: Record<string, unknown>
): Record<string, unknown> | undefined => {
  const extra: Record<string, unknown> = { ...(base || {}) }
  const maxBody = Number(maxRequestBodyMB.value)
  if (Number.isFinite(maxBody) && maxBody > 0) {
    extra.max_request_body_mb = Math.trunc(maxBody)
  } else {
    delete extra.max_request_body_mb
  }
  return Object.keys(extra).length > 0 ? extra : undefined
}

const accountCreateGuide = computed(() => {
  if (accountCategory.value === 'service_account') {
    return {
      title: t('admin.accounts.createGuide.vertexService.title'),
      description: t('admin.accounts.createGuide.vertexService.description'),
      typeStep: t('admin.accounts.createGuide.vertexService.typeStep'),
      nextStep: t('admin.accounts.createGuide.vertexService.nextStep'),
      namePlaceholder: t('admin.accounts.createGuide.vertexService.namePlaceholder')
    }
  }

  if (form.platform === 'anthropic' && accountCategory.value === 'bedrock') {
    return {
      title: t('admin.accounts.createGuide.anthropicBedrock.title'),
      description: t('admin.accounts.createGuide.anthropicBedrock.description'),
      typeStep: t('admin.accounts.createGuide.anthropicBedrock.typeStep'),
      nextStep: t('admin.accounts.createGuide.anthropicBedrock.nextStep'),
      namePlaceholder: t('admin.accounts.createGuide.anthropicBedrock.namePlaceholder')
    }
  }

  if (form.platform === 'openai' && accountCategory.value === 'oauth-based') {
    return {
      title: t('admin.accounts.createGuide.openaiOauth.title'),
      description: t('admin.accounts.createGuide.openaiOauth.description'),
      typeStep: t('admin.accounts.createGuide.openaiOauth.typeStep'),
      nextStep: t('admin.accounts.createGuide.openaiOauth.nextStep'),
      namePlaceholder: t('admin.accounts.createGuide.openaiOauth.namePlaceholder')
    }
  }

  if (form.platform === 'openai' && accountCategory.value === 'apikey') {
    return {
      title: t('admin.accounts.createGuide.openaiApiKey.title'),
      description: t('admin.accounts.createGuide.openaiApiKey.description'),
      typeStep: t('admin.accounts.createGuide.openaiApiKey.typeStep'),
      nextStep: t('admin.accounts.createGuide.openaiApiKey.nextStep'),
      namePlaceholder: t('admin.accounts.createGuide.openaiApiKey.namePlaceholder')
    }
  }

  if (form.platform === 'gemini') {
    return {
      title: t('admin.accounts.createGuide.gemini.title'),
      description: t('admin.accounts.createGuide.gemini.description'),
      typeStep: t('admin.accounts.createGuide.gemini.typeStep'),
      nextStep: t('admin.accounts.createGuide.gemini.nextStep'),
      namePlaceholder: t('admin.accounts.createGuide.gemini.namePlaceholder')
    }
  }

  if (form.platform === 'antigravity') {
    return {
      title: t('admin.accounts.createGuide.antigravity.title'),
      description: t('admin.accounts.createGuide.antigravity.description'),
      typeStep: t('admin.accounts.createGuide.antigravity.typeStep'),
      nextStep: t('admin.accounts.createGuide.antigravity.nextStep'),
      namePlaceholder: t('admin.accounts.createGuide.antigravity.namePlaceholder')
    }
  }

  if (accountCategory.value === 'apikey') {
    return {
      title: t('admin.accounts.createGuide.anthropicApiKey.title'),
      description: t('admin.accounts.createGuide.anthropicApiKey.description'),
      typeStep: t('admin.accounts.createGuide.anthropicApiKey.typeStep'),
      nextStep: t('admin.accounts.createGuide.anthropicApiKey.nextStep'),
      namePlaceholder: t('admin.accounts.createGuide.anthropicApiKey.namePlaceholder')
    }
  }

  return {
    title: t('admin.accounts.createGuide.anthropicOauth.title'),
    description: t('admin.accounts.createGuide.anthropicOauth.description'),
    typeStep: t('admin.accounts.createGuide.anthropicOauth.typeStep'),
    nextStep: t('admin.accounts.createGuide.anthropicOauth.nextStep'),
    namePlaceholder: t('admin.accounts.createGuide.anthropicOauth.namePlaceholder')
  }
})
...
const requiresAccountName = computed(() => {
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      ...
      allowedModels.value = getDefaultAllowedModels(form.platform)
...
watch(
  ...
  if (newPlatform !== 'openai') {
      ...
      openaiAPIKeyMode.value = 'official'
...
const resetForm = () => {
  ...
  allowedModels.value = getDefaultAllowedModels('openai')
...
const handleSubmit = async () => {
  ...
  if (isOAuthFlow.value) {
    if (requiresAccountName.value && !form.name.trim()) {
  ...
  const extra = applyCommonSupplyExtra(buildAnthropicExtra(buildOpenAIExtra()))
...
const createAccountAndFinish = async (
  ...
  finalExtra = applyCommonSupplyExtra(finalExtra)
  await doCreateAccount({
    name: form.name,
    notes: form.notes,
    platform,
...
const handleOpenAIExchange = async (authCode: string) => {
  ...
  try {
    ...
    const extra = applyCommonSupplyExtra(buildOpenAIExtra(oauthExtra))
...
const handleOpenAIImportCodexSession = async (content: string) => {
  ...
  try {
    const extra = applyCommonSupplyExtra(buildOpenAIExtra())
    ...
    for (let i = 0; i < keys.length; i++) {
      try {
        ...
        await adminAPI.accounts.create({
          ...
          credentials,
          extra: applyCommonSupplyExtra(extra),
          proxy_id: form.proxy_id,
          concurrency: form.concurrency,
          load_factor: form.load_factor ?? undefined,
          priority: form.priority,
...
.account-create-guide {
  ...
  background:
    radial-gradient(circle at 0% 0%, rgba(238, 195, 73, 0.2), transparent 30%),
    linear-gradient(135deg, rgba(239, 250, 242, 0.94), rgba(255, 255, 255, 0.86));
  box-shadow: 0 14px 34px rgba(2, 43, 18, 0.06);
}

.account-create-guide__copy {
  min-width: 0;
}

.account-create-guide__eyebrow {
  display: inline-flex;
  width: fit-content;
  border-radius: 999px;
...
.account-create-guide__steps {
  display: grid;
  gap: 0.5rem;
}

.account-create-guide__steps span {
  display: flex;
  align-items: center;
  gap: 0.5rem;
...
:global(.dark) .account-create-guide {
  border-color: rgba(255, 255, 255, 0.1);
  background:
    radial-gradient(circle at 0% 0%, rgba(238, 195, 73, 0.1), transparent 30%),
    linear-gradient(135deg, rgba(0, 137, 78, 0.12), rgba(255, 255, 255, 0.05));
  box-shadow: 0 14px 34px rgba(0, 0, 0, 0.16);
}

:global(.dark) .account-create-guide__eyebrow,
:global(.dark) .account-create-guide__steps span {
  border-color: rgba(255, 255, 255, 0.1);
  background: rgba(255, 255, 255, 0.065);
}

:global(.dark) .account-create-guide h4 {
  color: rgba(255, 255, 255, 0.92);
}

:global(.dark) .account-create-guide p,
:global(.dark) .account-create-guide__steps span {
