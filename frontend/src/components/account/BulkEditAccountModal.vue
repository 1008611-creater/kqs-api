    ...
    <form id="bulk-edit-account-form" class="space-y-5" @submit.prevent="() => handleSubmit()">
      ...
      <div class="grid grid-cols-2 gap-4 border-t border-gray-200 pt-4 dark:border-dark-600 lg:grid-cols-5">
        ...
        <div>
          <div class="mb-3 flex items-center justify-between">
            <label
              id="bulk-edit-max-body-label"
              class="input-label mb-0"
              for="bulk-edit-max-body-enabled"
            >
              {{ t('admin.accounts.supplyProfile.maxBodyMB') }}
            </label>
            <input
              v-model="enableMaxRequestBody"
              id="bulk-edit-max-body-enabled"
              type="checkbox"
              aria-controls="bulk-edit-max-body"
              class="rounded border-gray-300 text-primary-600 focus:ring-primary-500"
            />
          </div>
          <input
            v-model.number="maxRequestBodyMB"
            id="bulk-edit-max-body"
            type="number"
            min="0"
            step="1"
            :disabled="!enableMaxRequestBody"
            class="input"
            :class="!enableMaxRequestBody && 'cursor-not-allowed opacity-50'"
            :placeholder="t('admin.accounts.supplyProfile.maxBodyPlaceholder')"
            aria-labelledby="bulk-edit-max-body-label"
...
const enableMaxRequestBody = ref(false)
...
const buildUpdatePayload = (): Record<string, unknown> | null => {
  ...
  if (enableMaxRequestBody.value) {
    const extra = ensureExtra()
    const maxBody = Number(maxRequestBodyMB.value)
    extra.max_request_body_mb = Number.isFinite(maxBody) && maxBody > 0
      ? Math.trunc(maxBody)
      : 0
  }
...
watch(
  ...
  (newShow) => {
    if (!newShow) {
      ...
      enableMaxRequestBody.value = false
