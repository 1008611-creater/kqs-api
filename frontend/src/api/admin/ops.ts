...
export interface OpsUpstreamTrafficTrendPoint {
  bucket_start: string
...
export interface OpsUpstreamTrafficAccountItem {
  account_id: number
  account_name: string
  platform: string
  request_bytes: number
...
export interface OpsUpstreamTrafficStatsResponse {
  start_time: string
  end_time: string
  platform?: string
  group_id?: number | null
  ...
  total_bytes: number
  request_count: number
  trend: OpsUpstreamTrafficTrendPoint[]
  top_accounts: OpsUpstreamTrafficAccountItem[]
}

export interface OpsProxyTrafficPoolItem {
  id: string
  name: string
  provider?: string
  enabled: boolean
  active: boolean
  configured_in_mihomo: boolean
  ...
  status: 'ok' | 'warning' | 'exhausted' | 'expired' | 'error' | 'unknown' | string
  message?: string
...
export interface OpsProxyTrafficPoolResponse {
  items: OpsProxyTrafficPoolItem[]
  total_bytes: number
  used_bytes: number
...
export interface OpsURLAllowlistSettings {
...
export async function getUpstreamTrafficStats(
  params: {
    time_range?: '5m' | '30m' | '1h' | '6h' | '24h'
    start_time?: string
    end_time?: string
  ...
  options: OpsRequestOptions = {}
): Promise<OpsUpstreamTrafficStatsResponse> {
  const { data } = await apiClient.get<OpsUpstreamTrafficStatsResponse>('/admin/ops/dashboard/upstream-traffic', {
    params,
    signal: options.signal
  })
  return data
...
export async function getProxyTrafficPools(
  options: OpsRequestOptions = {}
): Promise<OpsProxyTrafficPoolResponse> {
  const { data } = await apiClient.get<OpsProxyTrafficPoolResponse>('/admin/ops/dashboard/proxy-traffic-pools', {
    signal: options.signal
  })
  return data
}
...
// URL allowlist settings (DB-backed)
export async function getURLAllowlistSettings(): Promise<OpsURLAllowlistSettings> {
  const { data } = await apiClient.get<OpsURLAllowlistSettings>('/admin/ops/settings/url-allowlist')
  return data
}

export async function updateURLAllowlistSettings(config: OpsURLAllowlistSettings): Promise<OpsURLAllowlistSettings> {
  const { data } = await apiClient.put<OpsURLAllowlistSettings>('/admin/ops/settings/url-allowlist', config)
  return data
}
...
export const opsAPI = {
  ...
  getOpenAITokenStats,
  getUpstreamTrafficStats,
  getProxyTrafficPools,
  getConcurrencyStats,
  ...
  getURLAllowlistSettings,
  updateURLAllowlistSettings,
  getMetricThresholds,
