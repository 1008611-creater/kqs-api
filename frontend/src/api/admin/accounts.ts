...
export interface AccountSpeedTestRequest {
  account_ids?: number[]
  platform?: string
  type?: string
  status?: string
...
export interface AccountSpeedTestGroupRef {
  id: number
  name: string
}

export interface AccountSpeedTestItem {
  account_id: number
  name: string
  platform: string
  type: string
  ...
  schedulable: boolean
  group_ids: number[]
  groups: AccountSpeedTestGroupRef[]
...
export interface AccountSpeedTestSummary {
  total: number
  success: number
  failed: number
...
export interface AccountSpeedTestResponse {
  summary: AccountSpeedTestSummary
  results: AccountSpeedTestItem[]
  concurrency: number
  model_id?: string
}

export async function speedTest(
  request: AccountSpeedTestRequest
): Promise<AccountSpeedTestResponse> {
  const { data } = await apiClient.post<AccountSpeedTestResponse>(
    '/admin/accounts/speed-test',
    request,
    { timeout: 120000 }
  )
  return data
