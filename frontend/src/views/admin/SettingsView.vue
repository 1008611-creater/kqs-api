        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <div class="card">
            ...
            >
              <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    命中用户、API Key 或分组后，自动为 OpenAI/Codex 请求注入
                    <code class="rounded bg-gray-100 px-1 py-0.5 text-xs dark:bg-dark-700">service_tier=priority</code>
                    ，适合高价套餐或需要更快首 token 的客户。
                  </p>
                </div>
                <label class="relative inline-flex cursor-pointer items-center">
                  <input
                    v-model="openaiVIPFastModeForm.enabled"
                    type="checkbox"
                    class="peer sr-only"
                  />
                  <div
                    class="peer h-6 w-11 rounded-full bg-gray-200 after:absolute after:left-0.5 after:top-0.5 after:h-5 after:w-5 after:rounded-full after:bg-white after:transition-all after:content-[''] peer-checked:bg-primary-600 peer-checked:after:translate-x-full dark:bg-dark-600"
                  ></div>
                </label>
              </div>
            </div>
            <div class="space-y-5 p-6">
              ...
              >
                默认不会给全站开启。必须至少填一个用户 ID、API Key ID 或分组 ID；
                如果客户自己传了 service_tier，默认保留客户选择，避免误改低成本模式。
              </div>
              ...
              <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                <div>
                  ...
                  <textarea
                    v-model="openaiVIPFastModeForm.user_ids"
                    rows="3"
                    class="input"
                ...
                <div>
                  ...
                  <textarea
                    v-model="openaiVIPFastModeForm.api_key_ids"
                    rows="3"
                    class="input"
                ...
                <div>
                  ...
                  <textarea
                    v-model="openaiVIPFastModeForm.group_ids"
                    rows="3"
                    class="input"
                    placeholder="例如：2"
                  ></textarea>
              ...
              <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div>
                  ...
                  <textarea
                    v-model="openaiVIPFastModeForm.model_whitelist"
                    rows="3"
                    class="input"
                    placeholder="留空表示全部模型；可填 gpt-5.5 或 gpt-5.5*"
                  ></textarea>
                  <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">
                    一行一个或用逗号分隔，支持本项目已有的通配符写法。
                  </p>
                </div>
                <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
                  <label class="flex items-start gap-3">
                    <input
                      v-model="openaiVIPFastModeForm.respect_client_service_tier"
                      type="checkbox"
                    ...
                    <span>
                      ...
                      <span class="mt-1 block text-sm text-gray-500 dark:text-gray-400">
                        勾选后，如果客户主动传了 flex / priority / auto 等字段，后台不覆盖。
                        取消勾选会强制命中对象走 priority。
                      </span>
                    </span>
                  </label>
                </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      {{
                        localText(
                          "文档名称可自定义，内容按 Markdown 保存。可参考：服务条款、隐私说明、退款与卡密规则、可接受使用政策。",
              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    >
                      {{
                        (form.payment_product_name_prefix || "矿泉水API") +
                        " 100 " +
...
import { KQS_LEGAL_DOCUMENTS } from "@/constants/legalDocuments";
...
const openaiVIPFastModeForm = reactive({
  enabled: false,
  user_ids: "",
  api_key_ids: "",
  group_ids: "",
  model_whitelist: "",
  respect_client_service_tier: true,
});
const openaiVIPFastModeLoaded = ref(false);
...
function defaultLoginAgreementDocuments(): LoginAgreementDocument[] {
  return KQS_LEGAL_DOCUMENTS.map((doc) => ({ ...doc }));
}

function formatIDList(values?: number[]): string {
  return Array.isArray(values) ? values.filter((v) => v > 0).join(", ") : "";
}
...
function applyOpenAIVIPFastModeSettings(
  settings?: OpenAIVIPFastModeSettings,
) {
  if (!settings) return;
  openaiVIPFastModeForm.enabled = Boolean(settings.enabled);
  openaiVIPFastModeForm.user_ids = formatIDList(settings.user_ids);
  openaiVIPFastModeForm.api_key_ids = formatIDList(settings.api_key_ids);
  openaiVIPFastModeForm.group_ids = formatIDList(settings.group_ids);
  openaiVIPFastModeForm.model_whitelist = formatStringList(
    settings.model_whitelist,
  );
  openaiVIPFastModeForm.respect_client_service_tier =
    settings.respect_client_service_tier !== false;
  openaiVIPFastModeLoaded.value = true;
}
...
const form = reactive<SettingsForm>({
  ...
  login_agreement_updated_at: "2026-05-25",
...
async function loadSettings() {
  ...
  try {
    ...
    form.login_agreement_updated_at =
      settings.login_agreement_updated_at || "2026-05-25";
    ...
    if (settings.openai_vip_fast_mode_settings) {
      applyOpenAIVIPFastModeSettings(settings.openai_vip_fast_mode_settings);
    }
...
async function saveSettings() {
  ...
  try {
    ...
    if (openaiVIPFastModeLoaded.value) {
      payload.openai_vip_fast_mode_settings = {
        enabled: openaiVIPFastModeForm.enabled,
        user_ids: parseIDList(openaiVIPFastModeForm.user_ids),
        api_key_ids: parseIDList(openaiVIPFastModeForm.api_key_ids),
        group_ids: parseIDList(openaiVIPFastModeForm.group_ids),
        model_whitelist: parseStringList(
          openaiVIPFastModeForm.model_whitelist,
        ),
        respect_client_service_tier:
          openaiVIPFastModeForm.respect_client_service_tier,
      };
    }
    ...
    for (const [key, value] of Object.entries(updated)) {
      ...
      if (key === "openai_vip_fast_mode_settings") continue;
      if (value !== null && value !== undefined) {
        (form as Record<string, unknown>)[key] = value;
      }
    }
    ...
    if (updated.openai_vip_fast_mode_settings) {
      applyOpenAIVIPFastModeSettings(updated.openai_vip_fast_mode_settings);
    }
...
.settings-tab-active::after {
  ...
  content: "";
  background: linear-gradient(90deg, #00873c, #d9a21b);
}
