    ...
    <TablePageLayout>
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            <template v-for="(value, attrId) in activeAttributeFilters" :key="attrId">
              <div
                v-if="visibleFilters.has(`attr_${attrId}`) && isVisibleAttributeId(Number(attrId))"
      ...
      <template #table>
        ...
        >
          ...
          <template
            v-for="def in visibleAttributeDefinitions"
            :key="def.id"
...
const SYSTEM_ATTRIBUTE_KEYS = new Set(['onboarding_state', 'user_onboarding_state'])
const SYSTEM_ATTRIBUTE_NAMES = new Set(['矿泉水API 新手进度'])

const isSystemAttributeDefinition = (def: UserAttributeDefinition): boolean =>
  SYSTEM_ATTRIBUTE_KEYS.has(def.key) || SYSTEM_ATTRIBUTE_NAMES.has(def.name)

const visibleAttributeDefinitions = computed(() =>
  attributeDefinitions.value.filter(def => def.enabled && !isSystemAttributeDefinition(def))
)
const visibleAttributeIds = computed(() => new Set(visibleAttributeDefinitions.value.map(def => def.id)))
const isVisibleAttributeId = (attrId: number): boolean => visibleAttributeIds.value.has(attrId)

// Generate dynamic attribute columns from enabled definitions
const attributeColumns = computed<Column[]>(() =>
  visibleAttributeDefinitions.value
    .map(def => ({
      key: `attr_${def.id}`,
      label: def.name,
      sortable: false
...
const hasVisibleAttributeColumns = computed(() =>
  visibleAttributeDefinitions.value.some((def) => !hiddenColumns.has(`attr_${def.id}`))
)
...
const saveFiltersToStorage = () => {
  try {
    const visibleFilterKeys = [...visibleFilters].filter((key) => {
      if (!key.startsWith('attr_')) return true
      return isVisibleAttributeId(Number(key.replace('attr_', '')))
    })
    const visibleAttributeFilters = Object.fromEntries(
      Object.entries(activeAttributeFilters).filter(([attrId]) => isVisibleAttributeId(Number(attrId)))
    )
    // Save visible filters
    localStorage.setItem(VISIBLE_FILTERS_KEY, JSON.stringify(visibleFilterKeys))
    // Save filter values
    const values = {
      role: filters.role,
      status: filters.status,
      group: filters.group,
      attributes: visibleAttributeFilters
    }
...
const loadAttributeDefinitions = async () => {
  try {
    ...
    pruneHiddenAttributeFilters()
  } catch (e) {
    console.error('Failed to load attribute definitions:', e)
  }
}

const pruneHiddenAttributeFilters = () => {
  for (const key of [...visibleFilters]) {
    if (key.startsWith('attr_') && !isVisibleAttributeId(Number(key.replace('attr_', '')))) {
      visibleFilters.delete(key)
    }
  }
  for (const attrId of Object.keys(activeAttributeFilters)) {
    if (!isVisibleAttributeId(Number(attrId))) {
      delete activeAttributeFilters[Number(attrId)]
    }
  }
...
const loadUsers = async () => {
  ...
  try {
    ...
    for (const [attrId, value] of Object.entries(activeAttributeFilters)) {
      ...
      if (value && isVisibleAttributeId(numericAttrId)) {
        attrFilters[numericAttrId] = value
      }
    }
