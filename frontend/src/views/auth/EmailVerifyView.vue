...
const verifyCodeCooldownStorageKey = 'sub2api:auth:verify-code-cooldowns'
...
const siteName = ref<string>('矿泉水API')
...
onMounted(async () => {
  ...
  try {
    ...
    siteName.value = settings.site_name || '矿泉水API'
  } catch (error) {
  ...
  if (hasRegisterData.value) {
    const restoredCountdown = restoreStoredCountdown()
    if (restoredCountdown > 0) {
      codeSent.value = true
      startCountdown(restoredCountdown, { persist: false })
    } else {
      await sendCode()
    }
  }
...
function normalizeCooldownEmail(value: string): string {
  return value.trim().toLowerCase()
}

function readCooldownStore(): CooldownStore {
  try {
    const raw = sessionStorage.getItem(verifyCodeCooldownStorageKey)
    if (!raw) {
      return {}
    }
...
function writeCooldownStore(store: CooldownStore): void {
  const activeEntries = Object.fromEntries(
    Object.entries(store).filter(([, expiresAt]) => Number.isFinite(expiresAt) && expiresAt > Date.now())
  )
  if (Object.keys(activeEntries).length === 0) {
    sessionStorage.removeItem(verifyCodeCooldownStorageKey)
    return
  }
  sessionStorage.setItem(verifyCodeCooldownStorageKey, JSON.stringify(activeEntries))
}

function setStoredCountdown(seconds: number): void {
  const key = normalizeCooldownEmail(email.value)
  if (!key || seconds <= 0) {
    return
  }

  const store = readCooldownStore()
  store[key] = Date.now() + seconds * 1000
  writeCooldownStore(store)
}

function getStoredCountdownSeconds(): number {
  const key = normalizeCooldownEmail(email.value)
  if (!key) {
    return 0
  }
...
function clearStoredCountdown(): void {
  const key = normalizeCooldownEmail(email.value)
  if (!key) {
    return
  }
...
function restoreStoredCountdown(): number {
  return getStoredCountdownSeconds()
}

function startCountdown(seconds: number, options: { persist?: boolean } = {}): void {
  const normalizedSeconds = Math.max(0, Math.ceil(seconds || verifyCodeCooldownSeconds))
  countdown.value = normalizedSeconds

  if (countdownTimer) {
    clearInterval(countdownTimer)
  }

  if (options.persist !== false && normalizedSeconds > 0) {
    setStoredCountdown(normalizedSeconds)
  }
  ...
  countdownTimer = setInterval(() => {
    const storedRemaining = getStoredCountdownSeconds()
    if (storedRemaining > 0) {
      countdown.value = storedRemaining
      return
    }
...
function isCooldownError(error: unknown): boolean {
  ...
  return err.response?.status === 429 || message.includes('verify_code_too_frequent')
}
...
async function sendCode(): Promise<void> {
  ...
  try {
    ...
    codeSent.value = true
    startCountdown(Math.max(response.countdown || 0, verifyCodeCooldownSeconds))
    ...
    if (isCooldownError(error)) {
      startCountdown(verifyCodeCooldownSeconds)
    }
  } finally {
