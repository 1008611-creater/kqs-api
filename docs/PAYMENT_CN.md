...
| **Airwallex（空中云汇）** | 国际银行卡 / 本地支付方式 | 国际支付，支持多币种和托管收银台 |
...
国际支付平台，适合需要海外银行卡、本地支付方式和多币种结算的站点。后台添加服务商实例时，服务商选择 **Airwallex**，支付方式选择 `Airwallex`。

| 参数 | 说明 | 必填 |
|------|------|------|
| **Client ID** | Airwallex Scoped API Key 的 Client ID | 是 |
| **API Key** | Airwallex Scoped API Key | 是 |
| **Webhook Secret** | Airwallex Webhook 签名密钥 | 是 |
| **API Base** | 沙箱填 `https://api-demo.airwallex.com/api/v1`，生产填 `https://api.airwallex.com/api/v1` | 是 |
| **Country Code** | 收银台国家/地区代码，例如 `CN`、`HK`、`US` | 是 |
| **Currency** | 支付币种，例如 `CNY`、`HKD`、`USD` | 是 |
| **Account ID** | 多账户场景下的 Airwallex Account ID，单账户密钥可留空 | 否 |

创建 Scoped API Key 时，建议只给 **Payment Acceptance** 读取和写入权限；不要授予 Payouts、Transfers、Spend 等与在线收款无关的权限。
...
1. 登录 Airwallex 后台
2. 添加 Webhook 端点，填写回调地址
3. 至少订阅 `payment_intent.succeeded`，建议同时订阅 `payment_intent.cancelled`
4. 将生成的 Webhook Secret 填入服务商配置
