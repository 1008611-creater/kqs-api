...
func TestProviderCanHandlePaymentTypeRestrictsProviderFamilies(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:        "airwallex only matches airwallex provider",
			providerKey: TypeAirwallex,
			supported:   TypeAirwallex,
			paymentType: TypeAirwallex,
			want:        true,
		},
		{
			name:        "empty alipay provider does not catch airwallex",
		...
		{
			name:        "stripe only matches stripe provider",
		...
		{
			name:        "empty easypay provider does not catch stripe",
		...
		{
			name:        "easypay can still serve alipay",
			providerKey: TypeEasyPay,
			supported:   TypeAlipay,
			paymentType: TypeAlipay,
			want:        true,
		},
		{
			name:        "stripe submethod maps to stripe provider",
			providerKey: TypeStripe,
			supported:   TypeCard,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			inst := testInstance(1, tt.providerKey, "")
			inst.SupportedTypes = tt.supported
			got := providerCanHandlePaymentType(inst, tt.paymentType)
			if got != tt.want {
				t.Fatalf("providerCanHandlePaymentType(%q, %q, %q) = %v, want %v",
					tt.providerKey, tt.supported, tt.paymentType, got, tt.want)
			}
		})
	}
}
