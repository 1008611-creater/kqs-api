...
func (lb *DefaultLoadBalancer) queryEnabledInstances(
	...
	for _, inst := range instances {
		if providerCanHandlePaymentType(inst, paymentType) {
...
func providerCanHandlePaymentType(inst *dbent.PaymentProviderInstance, paymentType PaymentType) bool {
	if inst == nil {
		return false
	}
	switch paymentType {
	case TypeStripe:
		return inst.ProviderKey == TypeStripe
	case TypeCard, TypeLink:
		return inst.ProviderKey == TypeStripe && InstanceSupportsType(inst.SupportedTypes, paymentType)
	case TypeAirwallex:
		return inst.ProviderKey == TypeAirwallex && InstanceSupportsType(inst.SupportedTypes, paymentType)
	}

	switch normalizeVisibleMethodSupportType(paymentType) {
	case TypeAlipay:
		return (inst.ProviderKey == TypeAlipay || inst.ProviderKey == TypeEasyPay) && InstanceSupportsType(inst.SupportedTypes, paymentType)
	case TypeWxpay:
		return (inst.ProviderKey == TypeWxpay || inst.ProviderKey == TypeEasyPay) && InstanceSupportsType(inst.SupportedTypes, paymentType)
	default:
