...
func TestValidateURLFormat(t *testing.T) {
	...
	if _, err := ValidateURLFormat("https://localhost", true); err == nil {
		t.Fatalf("expected localhost to fail by default")
	}
	if _, err := ValidateURLFormat("https://127.0.0.1", true); err == nil {
		t.Fatalf("expected loopback ip to fail by default")
	}
	if _, err := ValidateURLFormat("https://10.0.0.1", true); err == nil {
		t.Fatalf("expected private ip to fail by default")
	}
	if _, err := ValidateURLFormatWithOptions("https://10.0.0.1", true, ValidationOptions{AllowPrivate: true}); err != nil {
		t.Fatalf("expected private ip to pass when AllowPrivate is true, got %v", err)
	}
