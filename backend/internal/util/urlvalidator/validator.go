...
func ValidateURLFormat(raw string, allowInsecureHTTP bool) (string, error) {
	return ValidateURLFormatWithOptions(raw, allowInsecureHTTP, ValidationOptions{})
}

func ValidateURLFormatWithOptions(raw string, allowInsecureHTTP bool, opts ValidationOptions) (string, error) {
	// Minimal validation used when allowlist is disabled. It still blocks
	// obvious local/private literal hosts unless explicitly allowed.
	trimmed := strings.TrimSpace(raw)
	if trimmed == "" {
		return "", errors.New("url is required")
	}
