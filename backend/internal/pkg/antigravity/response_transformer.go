	...
	if len(geminiResp.Candidates) > 0 {
		...
		if finishReason == "MALFORMED_FUNCTION_CALL" {
			...
			if geminiResp.Candidates[0].Content != nil {
				partCount = len(geminiResp.Candidates[0].Content.Parts)
			}
			log.Printf("[Antigravity] MALFORMED_FUNCTION_CALL detected in response for model %s parts=%d", originalModel, partCount)
		}
	}
