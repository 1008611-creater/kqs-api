...
func ProvideRouter(
	...
	configureTrustedClientIP(r, cfg)
...
func configureTrustedClientIP(r *gin.Engine, cfg *config.Config) {
	r.TrustedPlatform = normalizeTrustedPlatform(cfg.Server.TrustedPlatform)
	...
	if cfg.Server.Mode == "release" && r.TrustedPlatform == "" {
		log.Printf("Warning: server.trusted_proxies is empty in release mode; client IP trust chain is disabled")
	}
}

func normalizeTrustedPlatform(value string) string {
	switch strings.ToLower(strings.TrimSpace(value)) {
	case "", "none":
		return ""
	case "cloudflare", strings.ToLower(gin.PlatformCloudflare):
...
func ProvideHTTPServer(cfg *config.Config, router *gin.Engine) *http.Server {
	...
	server := &http.Server{
		...
		// MaxHeaderBytes: 显式限制请求头大小，避免异常大 header 消耗内存。
