...
const (
	maxPricingJSONResponseBytes = int64(8 << 20)
	maxPricingHashResponseBytes = int64(1 << 20)
)
...
func (c *pricingRemoteClient) FetchPricingJSON(ctx context.Context, url string) ([]byte, error) {
	...
	body, err := io.ReadAll(io.LimitReader(resp.Body, maxPricingJSONResponseBytes+1))
	if err != nil {
		return nil, err
	}
	if int64(len(body)) > maxPricingJSONResponseBytes {
		return nil, fmt.Errorf("pricing response exceeds limit: %d bytes", maxPricingJSONResponseBytes)
	}
	return body, nil
}
...
func (c *pricingRemoteClient) FetchHashText(ctx context.Context, url string) (string, error) {
	...
	body, err := io.ReadAll(io.LimitReader(resp.Body, maxPricingHashResponseBytes+1))
	if err != nil {
		return "", err
	}
	if int64(len(body)) > maxPricingHashResponseBytes {
		return "", fmt.Errorf("pricing hash response exceeds limit: %d bytes", maxPricingHashResponseBytes)
	}
