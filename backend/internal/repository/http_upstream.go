...
const (
	...
	// defaultDialTimeout: 默认 TCP 建连超时，避免冷连接在网络异常时长时间挂起
	defaultDialTimeout = 10 * time.Second
	// defaultTCPKeepAlive: 默认 TCP keepalive，用于保持上游热连接可复用
	defaultTCPKeepAlive = 30 * time.Second
	// defaultTLSHandshakeTimeout: 默认 TLS 握手超时，避免冷连接卡住首包
	defaultTLSHandshakeTimeout = 10 * time.Second
	// defaultExpectContinueTimeout: 默认 Expect-Continue 等待时间
...
func normalizeProxyURL(raw string) (string, *url.URL, error) {
	...
	if parsed == nil {
		if envProxy := firstNonEmptyEnv("HTTPS_PROXY", "https_proxy", "HTTP_PROXY", "http_proxy"); envProxy != "" {
			_, parsed, err = proxyurl.Parse(envProxy)
			if err != nil {
				return "", nil, fmt.Errorf("parse environment proxy: %w", err)
			}
		}
	}
	if parsed == nil {
