...
func (s *OpenAIOAuthServiceSuite) TestNonSuccessStatus_IncludesBody() {
	s.setupServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = io.WriteString(w, `{"error":"bad","access_token":"secret-token"}`)
	}))
	...
	require.NotContains(s.T(), err.Error(), "secret-token")
	require.Contains(s.T(), infraerrors.Message(err), `"access_token":"***"`)
}
