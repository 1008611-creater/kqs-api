...
func (s *PricingServiceSuite) TestFetchPricingJSON_RejectsOversizedBody() {
	s.setupServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write(bytes.Repeat([]byte("x"), int(maxPricingJSONResponseBytes)+1))
	}))

	_, err := s.client.FetchPricingJSON(s.ctx, s.srv.URL+"/large")
	require.Error(s.T(), err, "expected error for oversized pricing JSON")
	require.Contains(s.T(), err.Error(), "exceeds limit")
}
...
func (s *PricingServiceSuite) TestFetchHashText_RejectsOversizedBody() {
	s.setupServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write(bytes.Repeat([]byte("x"), int(maxPricingHashResponseBytes)+1))
	}))

	_, err := s.client.FetchHashText(s.ctx, s.srv.URL+"/large-hash")
	require.Error(s.T(), err, "expected error for oversized hash response")
	require.Contains(s.T(), err.Error(), "exceeds limit")
}
