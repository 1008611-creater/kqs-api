...
func (s *claudeOAuthService) GetAuthorizationCode(ctx context.Context, sessionKey, orgUUID, scope, codeChallenge, state, proxyURL string) (string, error) {
	...
	logger.LegacyPrintf("repository.claude_oauth", "[OAuth] Step 2 Request Body: response_type=%s client_id_present=%t organization_uuid_present=%t redirect_uri_present=%t scope_present=%t state_present=%t code_challenge_present=%t",
		reqBody["response_type"], reqBody["client_id"] != "", reqBody["organization_uuid"] != "", reqBody["redirect_uri"] != "", reqBody["scope"] != "", reqBody["state"] != "", reqBody["code_challenge"] != "")
...
func (s *claudeOAuthService) ExchangeCodeForToken(ctx context.Context, code, codeVerifier, state, proxyURL string, isSetupToken bool) (*oauth.TokenResponse, error) {
	...
	logger.LegacyPrintf("repository.claude_oauth", "[OAuth] Step 3 Request Body: grant_type=%s client_id_present=%t redirect_uri_present=%t code_present=%t code_verifier_present=%t state_present=%t setup_token=%t",
		reqBody["grant_type"], reqBody["client_id"] != "", reqBody["redirect_uri"] != "", reqBody["code"] != "", reqBody["code_verifier"] != "", reqBody["state"] != nil, isSetupToken)
