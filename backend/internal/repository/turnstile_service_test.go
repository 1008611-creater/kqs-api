...
func TestTurnstileProxyURLFromEnv(t *testing.T) {
	resetTurnstileProxyEnvForTest(t)
	t.Setenv("HTTP_PROXY", "http://http-proxy.local:7890")
	t.Setenv("HTTPS_PROXY", " http://https-proxy.local:7890 ")

	require.Equal(t, "http://https-proxy.local:7890", turnstileProxyURLFromEnv())
}

func TestTurnstileProxyURLFromEnv_HTTPFallback(t *testing.T) {
	resetTurnstileProxyEnvForTest(t)
	t.Setenv("HTTP_PROXY", "http://http-proxy.local:7890")

	require.Equal(t, "http://http-proxy.local:7890", turnstileProxyURLFromEnv())
}

func TestTurnstileProxyURLFromEnv_Fallbacks(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Windows treats environment keys case-insensitively")
	}
	resetTurnstileProxyEnvForTest(t)
	t.Setenv("http_proxy", " http://lower-http-proxy.local:7890 ")

	require.Equal(t, "http://lower-http-proxy.local:7890", turnstileProxyURLFromEnv())
}

func resetTurnstileProxyEnvForTest(t *testing.T) {
	t.Helper()
