...
func (s *HTTPUpstreamSuite) TestDefaultTransportKeepsWarmConnections() {
	svc := s.newService()
	entry := mustGetOrCreateClient(s.T(), svc, "", 0, 0)
	transport, ok := entry.client.Transport.(*http.Transport)
	require.True(s.T(), ok, "expected *http.Transport")
	require.NotNil(s.T(), transport.DialContext, "DialContext should set dial and TCP keepalive timeouts")
	require.Equal(s.T(), 10*time.Second, transport.TLSHandshakeTimeout, "TLSHandshakeTimeout mismatch")
	require.Equal(s.T(), time.Second, transport.ExpectContinueTimeout, "ExpectContinueTimeout mismatch")
	require.True(s.T(), transport.ForceAttemptHTTP2, "HTTP/2 should be attempted for HTTPS upstream reuse")
}
...
func (s *HTTPUpstreamSuite) TestNormalizeProxyURL_UsesEnvironmentProxyWhenEmpty() {
	s.T().Setenv("HTTP_PROXY", "")
	s.T().Setenv("http_proxy", "")
	s.T().Setenv("HTTPS_PROXY", "http://proxy.env:7890")

	key, parsed, err := normalizeProxyURL("")
	require.NoError(s.T(), err)
	require.NotNil(s.T(), parsed)
	require.Equal(s.T(), "http://proxy.env:7890", key)
}
...
func (s *HTTPUpstreamSuite) TestDo_EmptyAccountProxyUsesEnvironmentProxy() {
	seen := make(chan string, 1)
	proxySrv := newLocalTestServer(s.T(), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		seen <- r.RequestURI
		_, _ = io.WriteString(w, "env-proxied")
	}))
	s.T().Cleanup(proxySrv.Close)

	s.T().Setenv("HTTP_PROXY", "")
	s.T().Setenv("http_proxy", "")
	s.T().Setenv("HTTPS_PROXY", proxySrv.URL)
	...
	require.Error(s.T(), err, "mock proxy does not implement CONNECT tunneling, but it must be reached")
	select {
	case uri := <-seen:
		require.Equal(s.T(), "example.com:443", uri, "expected HTTPS request to use environment proxy tunnel")
	default:
		require.Fail(s.T(), "expected environment proxy to receive request")
	}
}
