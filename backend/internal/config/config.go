...
type ServerConfig struct {
	...
	MaxHeaderBytes     int       `mapstructure:"max_header_bytes"`      // 最大请求头大小
	TrustedProxies     []string  `mapstructure:"trusted_proxies"`       // 可信代理列表（CIDR/IP）
	TrustedPlatform    string    `mapstructure:"trusted_platform"`      // 可信 CDN 平台头，如 cloudflare
...
type SecurityConfig struct {
	...
	urlAllowlistUpstreamHostsLive    atomic.Value         `mapstructure:"-"`
}
...
func (c *Config) SetURLAllowlistUpstreamHosts(hosts []string) {
	if c == nil {
		return
	}
	cloned := append([]string(nil), hosts...)
	c.Security.URLAllowlist.UpstreamHosts = cloned
	c.Security.urlAllowlistUpstreamHostsLive.Store(cloned)
}

func (c *Config) URLAllowlistUpstreamHosts() []string {
	if c == nil {
		return nil
	}
	if v := c.Security.urlAllowlistUpstreamHostsLive.Load(); v != nil {
		if hosts, ok := v.([]string); ok {
			return append([]string(nil), hosts...)
		}
	}
	return append([]string(nil), c.Security.URLAllowlist.UpstreamHosts...)
}
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	cfg.Server.TrustedPlatform = strings.TrimSpace(cfg.Server.TrustedPlatform)
...
func setDefaults() {
	...
	viper.SetDefault("server.trusted_platform", "")
