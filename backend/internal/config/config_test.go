...
func TestLoadURLAllowlistUpstreamHostsFromCommaSeparatedEnv(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("SECURITY_URL_ALLOWLIST_ENABLED", "true")
	t.Setenv("SECURITY_URL_ALLOWLIST_UPSTREAM_HOSTS", "api.openai.com,auth.openai.com,chatgpt.com")

	cfg, err := Load()
	require.NoError(t, err)

	require.True(t, cfg.Security.URLAllowlist.Enabled)
	require.Equal(t, []string{"api.openai.com", "auth.openai.com", "chatgpt.com"}, cfg.Security.URLAllowlist.UpstreamHosts)
}
