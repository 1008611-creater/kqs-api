...
func (h *GatewayHandler) handleCCFailoverExhausted(c *gin.Context, lastErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	var clientUpstreamMsg string
	if lastErr != nil && lastErr.StatusCode > 0 {
		statusCode = lastErr.StatusCode
		clientUpstreamMsg = service.ExtractUpstreamErrorMessageForClient(lastErr.ResponseBody)
	}
	...
	if clientUpstreamMsg != "" {
		service.SetOpsUpstreamError(c, statusCode, clientUpstreamMsg, "")
		h.chatCompletionsErrorResponse(c, statusCode, "server_error", clientUpstreamMsg)
		return
	}
