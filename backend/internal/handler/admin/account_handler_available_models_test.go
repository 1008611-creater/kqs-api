...
func TestAccountHandlerGetAvailableModels_OpenAIUsesSyncedSupportedModels(t *testing.T) {
	svc := &availableModelsAdminService{
		stubAdminService: newStubAdminService(),
		account: service.Account{
			ID:       44,
	...
	require.Equal(t, "qwen3.6-flash", resp.Data[0].ID)
	require.Equal(t, "deepseek-v4-pro", resp.Data[1].ID)
	require.Equal(t, "GLM-5.1", resp.Data[2].ID)
	require.Equal(t, "GLM-5.1", resp.Data[2].DisplayName)
}
