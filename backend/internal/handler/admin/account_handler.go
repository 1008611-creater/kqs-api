...
type AccountSpeedTestRequest struct {
	AccountIDs      []int64 `json:"account_ids"`
	Platform        string  `json:"platform"`
	Type            string  `json:"type"`
	Status          string  `json:"status"`
...
const adminChannelSpeedTestModelID = "gpt-5.5"

type AccountSpeedTestGroupRef struct {
	ID   int64  `json:"id"`
	Name string `json:"name"`
}

type AccountSpeedTestItem struct {
	AccountID      int64                      `json:"account_id"`
	Name           string                     `json:"name"`
	Platform       string                     `json:"platform"`
	Type           string                     `json:"type"`
	...
	Schedulable    bool                       `json:"schedulable"`
	GroupIDs       []int64                    `json:"group_ids"`
	Groups         []AccountSpeedTestGroupRef `json:"groups"`
...
type AccountSpeedTestSummary struct {
	Total          int     `json:"total"`
	Success        int     `json:"success"`
	Failed         int     `json:"failed"`
	AvgLatencyMs   int64   `json:"avg_latency_ms"`
	P95LatencyMs   int64   `json:"p95_latency_ms"`
	FastestID      int64   `json:"fastest_id,omitempty"`
	FastestName    string  `json:"fastest_name,omitempty"`
	FastestLatency int64   `json:"fastest_latency_ms,omitempty"`
	SlowestID      int64   `json:"slowest_id,omitempty"`
	SlowestName    string  `json:"slowest_name,omitempty"`
	SlowestLatency int64   `json:"slowest_latency_ms,omitempty"`
	SuccessRate    float64 `json:"success_rate"`
}

type AccountSpeedTestResponse struct {
	Summary     AccountSpeedTestSummary `json:"summary"`
	Results     []AccountSpeedTestItem  `json:"results"`
	Concurrency int                     `json:"concurrency"`
...
// SpeedTest runs a bounded batch connectivity test for admin routing decisions.
// POST /api/v1/admin/accounts/speed-test
func (h *AccountHandler) SpeedTest(c *gin.Context) {
	if h.accountTestService == nil {
		response.ErrorFrom(c, infraerrors.New(http.StatusServiceUnavailable, "ACCOUNT_TEST_UNAVAILABLE", "account test service unavailable"))
		return
	}

	var req AccountSpeedTestRequest
	_ = c.ShouldBindJSON(&req)
	req.Search = strings.TrimSpace(req.Search)
	req.ModelID = adminChannelSpeedTestModelID

	limit := clampAccountSpeedTestLimit(req.Limit)
	concurrency := clampAccountSpeedTestConcurrency(req.Concurrency)
	onlySchedulable := true
	if req.OnlySchedulable != nil {
		onlySchedulable = *req.OnlySchedulable
	}

	targets, err := h.resolveSpeedTestTargets(c.Request.Context(), req, limit)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	if onlySchedulable {
		filtered := make([]*service.Account, 0, len(targets))
	...
	results := make([]AccountSpeedTestItem, len(targets))
	...
	for i, account := range targets {
		...
		wg.Add(1)
		go func(index int, acc *service.Account) {
			defer wg.Done()
			sem <- struct{}{}
			defer func() { <-sem }()

			item := buildAccountSpeedTestItem(acc)
			startedAt := time.Now()
			item.StartedAtUnix = startedAt.Unix()

			testCtx, cancel := context.WithTimeout(baseCtx, 45*time.Second)
			defer cancel()

			result, testErr := h.accountTestService.RunTestBackground(testCtx, acc.ID, req.ModelID)
			finishedAt := time.Now()
			item.FinishedAtUnix = finishedAt.Unix()
			item.LatencyMs = finishedAt.Sub(startedAt).Milliseconds()

			if result != nil {
				item.LatencyMs = result.LatencyMs
				item.Success = result.Status == "success"
				item.Error = strings.TrimSpace(result.ErrorMessage)
			}
			if testErr != nil {
				item.Success = false
				if item.Error == "" {
			...
			if item.Success && item.LatencyMs <= 0 {
				item.LatencyMs = finishedAt.Sub(startedAt).Milliseconds()
			}
	...
	response.Success(c, AccountSpeedTestResponse{
		Summary:     summarizeAccountSpeedTest(results),
		Results:     results,
...
func (h *AccountHandler) resolveSpeedTestTargets(ctx context.Context, req AccountSpeedTestRequest, limit int) ([]*service.Account, error) {
...
func buildAccountSpeedTestItem(account *service.Account) AccountSpeedTestItem {
	groups := make([]AccountSpeedTestGroupRef, 0, len(account.Groups))
	for _, group := range account.Groups {
		if group == nil {
			continue
		}
		groups = append(groups, AccountSpeedTestGroupRef{
			ID:   group.ID,
			Name: group.Name,
		})
	}
...
func summarizeAccountSpeedTest(results []AccountSpeedTestItem) AccountSpeedTestSummary {
	summary := AccountSpeedTestSummary{Total: len(results)}
	if len(results) == 0 {
		return summary
	}
	...
	for _, item := range results {
		if item.Success {
			...
			if summary.FastestID == 0 || item.LatencyMs < summary.FastestLatency {
				summary.FastestID = item.AccountID
				summary.FastestName = item.Name
				summary.FastestLatency = item.LatencyMs
			}
			if summary.SlowestID == 0 || item.LatencyMs > summary.SlowestLatency {
				summary.SlowestID = item.AccountID
				summary.SlowestName = item.Name
				summary.SlowestLatency = item.LatencyMs
			}
		} else {
			summary.Failed++
		}
	...
	summary.AvgLatencyMs = sum / int64(len(latencies))
	...
	if p95Index > len(latencies) {
		p95Index = len(latencies)
	}
	summary.P95LatencyMs = latencies[p95Index-1]
	return summary
}

func clampAccountSpeedTestLimit(limit int) int {
	if limit <= 0 {
		return 30
	}
	if limit > 80 {
...
func clampAccountSpeedTestConcurrency(concurrency int) int {
	if concurrency <= 0 {
		return 3
	}
	if concurrency > 6 {
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	if account.IsOpenAI() {
		...
		if models := openAIModelsFromIDs(accountSupportedModelIDs(account)); len(models) > 0 {
	...
	if account.IsGemini() {
		...
		if models := geminiModelsFromIDs(accountSupportedModelIDs(account)); len(models) > 0 {
			response.Success(c, models)
			return
		}
	...
	if models := claudeModelsFromIDs(accountSupportedModelIDs(account)); len(models) > 0 {
		response.Success(c, models)
		return
	}
...
func accountSupportedModelIDs(account *service.Account) []string {
	...
	raw, ok := account.Extra["supported_models"]
	if !ok || raw == nil {
		return nil
	}
