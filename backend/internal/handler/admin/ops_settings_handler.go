...
// GetURLAllowlistSettings returns runtime-editable upstream URL allowlist.
// GET /api/v1/admin/ops/settings/url-allowlist
func (h *OpsHandler) GetURLAllowlistSettings(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	...
	cfg, err := h.opsService.GetURLAllowlistSettings(c.Request.Context())
	if err != nil {
		response.Error(c, http.StatusInternalServerError, "Failed to get url allowlist settings")
		return
	}
	response.Success(c, cfg)
}

// UpdateURLAllowlistSettings updates runtime-editable upstream URL allowlist.
// PUT /api/v1/admin/ops/settings/url-allowlist
func (h *OpsHandler) UpdateURLAllowlistSettings(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	...
	var req service.OpsURLAllowlistSettings
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request body")
		return
	}

	updated, err := h.opsService.UpdateURLAllowlistSettings(c.Request.Context(), &req)
	if err != nil {
		response.Error(c, http.StatusBadRequest, err.Error())
		return
	}
