...
// GetDashboardUpstreamTraffic returns admin-only upstream/proxy traffic stats.
// GET /api/v1/admin/ops/dashboard/upstream-traffic
func (h *OpsHandler) GetDashboardUpstreamTraffic(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	...
	bucketSeconds := pickThroughputBucketSeconds(endTime.Sub(startTime))
	data, err := h.opsService.GetUpstreamTrafficStats(c.Request.Context(), filter, bucketSeconds)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// GetDashboardProxyTrafficPools returns subscription traffic-pool status.
// GET /api/v1/admin/ops/dashboard/proxy-traffic-pools
func (h *OpsHandler) GetDashboardProxyTrafficPools(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	...
	data, err := h.opsService.GetProxyTrafficPools(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
