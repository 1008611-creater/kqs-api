...
func TestParseCodexSessionImportEntriesUnwrapsSub2APIExportAccounts(t *testing.T) {
	accessToken := buildCodexImportTestJWT(t, time.Now().Add(time.Hour), map[string]any{
		"email": "export@example.com",
	})
	req := CodexSessionImportRequest{
		Content: fmt.Sprintf(`{
			"version": "sub2api-test-export",
			"data": {
				"accounts": [
					{
						...
						"credentials": {
							"access_token": %q,
							"refresh_token": "rt_export_refresh",
							"client_id": "client-export",
	...
	if item.Credentials["access_token"] != accessToken {
		t.Fatalf("access_token not normalized from credentials")
	}
	if item.Credentials["refresh_token"] != "rt_export_refresh" {
		t.Fatalf("refresh_token = %v, want rt_export_refresh", item.Credentials["refresh_token"])
	}
	if item.Credentials["client_id"] != "client-export" {
		t.Fatalf("client_id = %v, want client-export", item.Credentials["client_id"])
	}
...
func TestNormalizeCodexAuthJSONOpenAIAPIKey(t *testing.T) {
	item, err := normalizeCodexImportEntry(codexImportEntry{
		Index: 1,
		Value: map[string]any{
			"OPENAI_API_KEY": "sk-codex-auth-json",
		},
	})
	if err != nil {
		t.Fatalf("normalizeCodexImportEntry error = %v", err)
	}
	if item.Credentials["access_token"] != "sk-codex-auth-json" {
		t.Fatalf("access_token = %v, want sk-codex-auth-json", item.Credentials["access_token"])
	}
}
...
func TestCodexImportRefreshOnlyInputDetectsRTForms(t *testing.T) {
	refreshToken, clientID, hasAccessToken := codexImportRefreshOnlyInput("rt_test_refresh")
	if refreshToken != "rt_test_refresh" || clientID != "" || hasAccessToken {
		t.Fatalf("naked rt detection = (%q, %q, %v)", refreshToken, clientID, hasAccessToken)
	}

	refreshToken, clientID, hasAccessToken = codexImportRefreshOnlyInput(map[string]any{
		"refresh_token": "rt_json_refresh",
		"client_id":     "client-json",
	})
	if refreshToken != "rt_json_refresh" || clientID != "client-json" || hasAccessToken {
		t.Fatalf("json rt detection = (%q, %q, %v)", refreshToken, clientID, hasAccessToken)
	}

	refreshToken, _, hasAccessToken = codexImportRefreshOnlyInput(map[string]any{
		"tokens": map[string]any{
			"access_token":  "access-token",
			"refresh_token": "rt_with_access",
		},
	})
	if refreshToken != "rt_with_access" || !hasAccessToken {
		t.Fatalf("full token detection = (%q, %v)", refreshToken, hasAccessToken)
	}

	refreshToken, clientID, hasAccessToken = codexImportRefreshOnlyInput(map[string]any{
		"tokens": map[string]any{
			"rt":       "rt_nested_alias",
			"clientId": "client-nested-alias",
		},
	})
	if refreshToken != "rt_nested_alias" || clientID != "client-nested-alias" || hasAccessToken {
		t.Fatalf("nested rt-only detection = (%q, %q, %v)", refreshToken, clientID, hasAccessToken)
	}
}

func TestBuildCodexImportValueFromRefreshNormalizesTokenInfo(t *testing.T) {
	accessToken := buildCodexImportTestJWT(t, time.Now().Add(time.Hour), map[string]any{
		"email": "refresh@example.com",
		"https://api.openai.com/auth": map[string]any{
			"chatgpt_account_id": "acct-refresh",
			"chatgpt_user_id":    "user-refresh",
		},
	})
	tokenInfo := &service.OpenAITokenInfo{
		...
		ClientID:         "client-from-refresh",
		Email:            "refresh@example.com",
		ChatGPTAccountID: "acct-refresh",
		ChatGPTUserID:    "user-refresh",
		OrganizationID:   "org-refresh",
		PlanType:         "plus",
	}

	value := buildCodexImportValueFromRefresh(
		map[string]any{"refresh_token": "rt_original", "name": "Original"},
		"rt_original",
		"client-default",
		tokenInfo,
	)
	...
	if item.Credentials["access_token"] != accessToken {
		t.Fatalf("access_token not normalized")
	}
	if item.Credentials["refresh_token"] != "rt_original" {
		t.Fatalf("refresh_token = %v, want rt_original", item.Credentials["refresh_token"])
	}
	if item.Credentials["client_id"] != "client-from-refresh" {
		t.Fatalf("client_id = %v, want client-from-refresh", item.Credentials["client_id"])
	}
	if item.Credentials["chatgpt_account_id"] != "acct-refresh" {
		t.Fatalf("chatgpt_account_id = %v, want acct-refresh", item.Credentials["chatgpt_account_id"])
	}
}

func TestNormalizeCodexSessionJSONAcceptsRtAliasWithAccessToken(t *testing.T) {
	accessToken := buildCodexImportTestJWT(t, time.Now().Add(time.Hour), map[string]any{
		"email": "rt-alias@example.com",
	})
	...
	if item.RefreshToken != "rt_alias_refresh" {
		t.Fatalf("RefreshToken = %q, want rt_alias_refresh", item.RefreshToken)
	}
	if item.Credentials["refresh_token"] != "rt_alias_refresh" {
		t.Fatalf("refresh_token = %v, want rt_alias_refresh", item.Credentials["refresh_token"])
	}
	if item.Credentials["client_id"] != "client-alias" {
		t.Fatalf("client_id = %v, want client-alias", item.Credentials["client_id"])
	}
}

func TestCodexImportSafeMessageRedactsSensitiveTokens(t *testing.T) {
	message := codexImportSafeMessage(errors.New(`refresh failed: {"access_token":"access-secret","refreshToken":"rt-secret","rt":"rt-alias","id_token":"id-secret","token":"plain-secret"}`))

	for _, secret := range []string{"access-secret", "rt-secret", "rt-alias", "id-secret", "plain-secret"} {
		if strings.Contains(message, secret) {
			t.Fatalf("message leaked %q: %s", secret, message)
		}
	}
	if !strings.Contains(message, "***") {
		t.Fatalf("message = %q, want redaction marker", message)
	}
}
