...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	auditOutcome := service.RegistrationAuditOutcomeFailed
	auditReason := "INVALID_REQUEST"
	var auditUserID int64
	defer func() {
		h.recordRegistrationAudit(c, service.RegistrationAuditActionRegister, req.Email, req.TurnstileToken, auditOutcome, auditReason, auditUserID)
	}()
	...
	if !validateRegistrationPassword(req.Password) {
		auditReason = "WEAK_PASSWORD"
		response.BadRequest(c, "密码至少需要 8 位，并且必须同时包含英文字母和数字")
		return
	}

	// Turnstile 验证（邮箱验证码注册场景避免重复校验一次性 token）
	if err := h.authService.VerifyTurnstileForRegister(c.Request.Context(), req.TurnstileToken, ip.GetClientIP(c), req.Email, req.VerifyCode); err != nil {
		auditReason = authAuditFailureReason(err, "TURNSTILE_VERIFICATION_FAILED")
		response.ErrorFrom(c, err)
		return
	}
	...
	if err != nil {
		auditReason = authAuditFailureReason(err, "REGISTER_FAILED")
		response.ErrorFrom(c, err)
		return
	}

	auditOutcome = service.RegistrationAuditOutcomeSuccess
	auditReason = ""
	if user != nil {
		auditUserID = user.ID
	}
...
func (h *AuthHandler) SendVerifyCode(c *gin.Context) {
	...
	auditReason := "INVALID_REQUEST"
	defer func() {
		h.recordRegistrationAudit(c, service.RegistrationAuditActionSendVerifyCode, req.Email, req.TurnstileToken, auditOutcome, auditReason, 0)
	}()
	...
	// 注册发码入口必须经过 Turnstile，避免注册机批量触发验证码邮件。
	if err := h.authService.VerifyTurnstileForRegister(c.Request.Context(), req.TurnstileToken, ip.GetClientIP(c), req.Email, ""); err != nil {
		auditReason = authAuditFailureReason(err, "TURNSTILE_VERIFICATION_FAILED")
		response.ErrorFrom(c, err)
		return
	}
	...
	if err != nil {
		auditReason = authAuditFailureReason(err, "SEND_VERIFY_CODE_FAILED")
		response.ErrorFrom(c, err)
		return
	}
...
func (h *AuthHandler) recordRegistrationAudit(c *gin.Context, action, email, turnstileToken, outcome, reason string, userID int64) {
	if h == nil || h.authService == nil || c == nil || c.Request == nil {
		return
	}
	h.authService.RecordRegistrationAudit(c.Request.Context(), service.RegistrationAuditInput{
		Action:           action,
		Outcome:          outcome,
		...
		TurnstilePresent: strings.TrimSpace(turnstileToken) != "",
	})
}

func authAuditFailureReason(err error, fallback string) string {
	if err == nil {
		return ""
	}
	if appErr := infraerrors.FromError(err); appErr != nil && appErr.Reason != "" {
