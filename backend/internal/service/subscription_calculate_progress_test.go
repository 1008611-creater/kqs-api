...
func TestCalculateProgress_DailyCardUsesDailyWindowResetTime(t *testing.T) {
	svc := newTestSubscriptionService()
	...
	assert.Equal(t, dailyStart.Add(24*time.Hour), progress.Daily.ResetsAt, "日卡额度刷新时间应为日窗口结束时间")
}
