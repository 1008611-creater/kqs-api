...
type openAIRequestBodySizeContextKey struct{}

// WithOpenAIRequestBodySize annotates a request context with the raw request
// body size so the scheduler can skip upstream accounts with small body limits.
func WithOpenAIRequestBodySize(ctx context.Context, size int64) context.Context {
	if ctx == nil || size <= 0 {
		return ctx
	}
	return context.WithValue(ctx, openAIRequestBodySizeContextKey{}, size)
}

func openAIRequestBodySizeFromContext(ctx context.Context) int64 {
	if ctx == nil {
		return 0
	}
	size, _ := ctx.Value(openAIRequestBodySizeContextKey{}).(int64)
	if size < 0 {
		return 0
	}
	return size
...
var ErrOpenAIClientDisconnected = errors.New("stream usage incomplete: openai client disconnected")
...
func isOpenAIAccountEligibleForRequest(account *Account, requestedModel string, requireCompact bool) bool {
	...
	if !account.IsOpenAIRequestedModelAllowed(requestedModel) {
		return false
	}
...
func accountAllowsOpenAIRequestBodySize(account *Account, requestBodyBytes int64) bool {
	if account == nil || requestBodyBytes <= 0 {
		return true
	}
	maxBytes := account.GetMaxRequestBodyBytes()
	return maxBytes <= 0 || requestBodyBytes <= maxBytes
}
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64) *Account {
	...
	if !accountAllowsOpenAIRequestBodySize(account, openAIRequestBodySizeFromContext(ctx)) {
		_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
		return nil
	}
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool) (*Account, bool) {
	...
	for i := range accounts {
		...
		if !accountAllowsOpenAIRequestBodySize(fresh, openAIRequestBodySizeFromContext(ctx)) {
			continue
		}
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool) (*AccountSelectionResult, error) {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky &&
					...
					accountAllowsOpenAIRequestBodySize(account, openAIRequestBodySizeFromContext(ctx)) {
					account = s.recheckSelectedOpenAIAccountFromDB(ctx, account, requestedModel, requireCompact)
					if account == nil {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
	...
	for i := range accounts {
		...
		if !acc.IsOpenAIRequestedModelAllowed(requestedModel) {
			continue
		}
		if !accountAllowsOpenAIRequestBodySize(acc, openAIRequestBodySizeFromContext(ctx)) {
			continue
		}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDB(ctx context.Context, account *Account, requestedModel string, requireCompact bool) *Account {
	...
	if !accountAllowsOpenAIRequestBodySize(latest, openAIRequestBodySizeFromContext(ctx)) {
		return nil
	}
...
func (s *OpenAIGatewayService) shouldFailoverOpenAIUpstreamResponse(statusCode int, upstreamMsg string, upstreamBody []byte) bool {
	...
	if isOpenAIUpstreamConcurrencyLimited(upstreamMsg, upstreamBody) {
		return true
	}
	return isOpenAITransientProcessingError(statusCode, upstreamMsg, upstreamBody)
}

func isOpenAIUpstreamConcurrencyLimited(upstreamMsg string, upstreamBody []byte) bool {
	combined := strings.ToLower(strings.TrimSpace(upstreamMsg + " " + string(upstreamBody)))
	if combined == "" {
		return false
	}
	return strings.Contains(combined, "concurrency limit exceeded") ||
		strings.Contains(combined, "too many concurrent") ||
		strings.Contains(combined, "concurrent request")
}
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if injected, err := s.applyOpenAIVIPFastModeToMap(ctx, account, upstreamModel, reqBody); err != nil {
		return nil, err
	} else if injected {
		bodyModified = true
		disablePatch()
	...
	for {
		...
		var upstreamResponseBytes int64
		imageCount := 0
		...
		if reqStream {
			...
			upstreamResponseBytes = streamResult.upstreamResponseBytes
			...
			upstreamResponseBytes = nonStreamResult.upstreamResponseBytes
...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	var vipErr error
	if body, _, vipErr = s.applyOpenAIVIPFastModeToBody(ctx, account, policyModel, body); vipErr != nil {
		return nil, vipErr
	}
	...
	if reqStream {
		...
		upstreamResponseBytes = result.upstreamResponseBytes
...
func (s *OpenAIGatewayService) handleErrorResponsePassthrough(
	...
	// Do not return raw upstream error bodies to clients. They often contain provider-specific
	// wording, internal routing clues, and occasionally upstream request metadata. Ops logs above
	// keep the sanitized detail for administrators.
	statusCode := resp.StatusCode
	...
	if resp.StatusCode == http.StatusTooManyRequests {
		clientMessage = "Upstream is temporarily rate limited"
	}
	c.JSON(statusCode, gin.H{
		"error": gin.H{
			"type":    "upstream_error",
...
func (s *OpenAIGatewayService) markOpenAIStreamDisconnectTempUnsched(
	ctx context.Context,
	account *Account,
	model string,
	message string,
	...
	if reason == "" {
		reason = "OpenAI stream disconnected before completion"
	}
	until := time.Now().Add(2 * time.Hour)
	if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, reason); err != nil {
		slog.Warn("openai_stream_disconnect_set_temp_unschedulable_failed", "account_id", account.ID, "error", err)
		return
	}
	s.BlockAccountScheduling(account, until, "stream_disconnect")
	slog.Warn("openai_stream_disconnect_temp_unschedulable", "account_id", account.ID, "until", until, "model", model, "reason", reason)
}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	for scanner.Scan() {
		line := scanner.Text()
		upstreamResponseBytes += int64(len(line) + 1)
...
func (s *OpenAIGatewayService) handleNonStreamingResponsePassthrough(
	...
	upstreamResponseBytes := int64(len(body))
...
func (s *OpenAIGatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string) (*openaiStreamingResult, error) {
	...
	finalizeStream := func() (*openaiStreamingResult, error) {
		if !sawTerminalEvent {
			if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
				s.markOpenAIStreamDisconnectTempUnsched(ctx, account, originalModel, "OpenAI stream ended before a terminal event")
	...
	handleScanErr := func(scanErr error) (*openaiStreamingResult, error, bool) {
		...
		if !openAIStreamClientOutputStarted(c, clientOutputStarted) {
			...
			s.markOpenAIStreamDisconnectTempUnsched(ctx, account, originalModel, msg)
			return resultWithUsage(), s.newOpenAIStreamFailoverError(c, account, false, upstreamRequestID, nil, msg), true
		}
		// 客户端已断开时立即结束，避免无接收方的上游流继续占用并发槽。
		if clientDisconnected {
			return resultWithUsage(), ErrOpenAIClientDisconnected, true
		}
		s.markOpenAIStreamDisconnectTempUnsched(ctx, account, originalModel, "OpenAI stream read error: "+scanErr.Error())
		sendErrorEvent("stream_read_error")
		return resultWithUsage(), fmt.Errorf("stream read error: %w", scanErr), true
	}
	processSSELine := func(line string, queueDrained bool) bool {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if !clientDisconnected {
				...
				if _, err := bufferedWriter.WriteString(line); err != nil {
					clientDisconnected = true
					logger.LegacyPrintf("service.openai_gateway", "Client disconnected during streaming, aborting upstream stream")
					return false
					...
					if err := flushBuffered(); err != nil {
						clientDisconnected = true
						logger.LegacyPrintf("service.openai_gateway", "Client disconnected during streaming flush, aborting upstream stream")
						return false
					} else {
						clientOutputStarted = true
						lastDownstreamWriteAt = time.Now()
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.line != "" {
				upstreamResponseBytes += int64(len(ev.line) + 1)
			}
			if !processSSELine(ev.line, len(events) == 0) {
				return resultWithUsage(), ErrOpenAIClientDisconnected
			}
			if streamFailoverErr != nil {
				return resultWithUsage(), streamFailoverErr
		...
		case <-keepaliveCh:
			...
			if err := flushBuffered(); err != nil {
				clientDisconnected = true
				logger.LegacyPrintf("service.openai_gateway", "Client disconnected during keepalive flush, aborting upstream stream")
				return resultWithUsage(), ErrOpenAIClientDisconnected
			}
			lastDownstreamWriteAt = time.Now()
		}
...
func (s *OpenAIGatewayService) validateUpstreamBaseURL(raw string) (string, error) {
	if s.cfg != nil && !s.cfg.Security.URLAllowlist.Enabled {
		normalized, err := urlvalidator.ValidateURLFormatWithOptions(raw, s.cfg.Security.URLAllowlist.AllowInsecureHTTP, urlvalidator.ValidationOptions{
	...
	normalized, err := urlvalidator.ValidateHTTPSURL(raw, urlvalidator.ValidationOptions{
		AllowedHosts:     s.cfg.URLAllowlistUpstreamHosts(),
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	usageLog := &UsageLog{
		...
		UpstreamRequestBytes:  result.UpstreamRequestBytes,
		UpstreamResponseBytes: result.UpstreamResponseBytes,
	}
	if cost != nil {
...
// OpenAIVIPFastModeTarget identifies the user-side credential currently
// making a request. The gateway uses it to decide whether to inject OpenAI's
// priority service tier for VIP traffic.
type OpenAIVIPFastModeTarget struct {
	UserID   int64
	APIKeyID int64
	GroupID  *int64
}

type openAIVIPFastModeTargetCtxKeyType struct{}
type openAIVIPFastModeSettingsCtxKeyType struct{}

var openAIVIPFastModeTargetCtxKey = openAIVIPFastModeTargetCtxKeyType{}
var openAIVIPFastModeSettingsCtxKey = openAIVIPFastModeSettingsCtxKeyType{}

// WithOpenAIVIPFastModeTarget binds a sanitized VIP matching target to the
// request context. It intentionally stores only numeric IDs, never secrets.
func WithOpenAIVIPFastModeTarget(ctx context.Context, target OpenAIVIPFastModeTarget) context.Context {
	if ctx == nil {
		return ctx
	}
	...
	return context.WithValue(ctx, openAIVIPFastModeTargetCtxKey, target)
}

func openAIVIPFastModeTargetFromContext(ctx context.Context) (OpenAIVIPFastModeTarget, bool) {
	if ctx == nil {
		return OpenAIVIPFastModeTarget{}, false
	}
	target, ok := ctx.Value(openAIVIPFastModeTargetCtxKey).(OpenAIVIPFastModeTarget)
	return target, ok
}

func withOpenAIVIPFastModeSettingsContext(ctx context.Context, settings *OpenAIVIPFastModeSettings) context.Context {
	if ctx == nil || settings == nil {
		return ctx
	}
	return context.WithValue(ctx, openAIVIPFastModeSettingsCtxKey, settings)
}

func openAIVIPFastModeSettingsFromContext(ctx context.Context) *OpenAIVIPFastModeSettings {
	if ctx == nil {
		return nil
	}
	if v, ok := ctx.Value(openAIVIPFastModeSettingsCtxKey).(*OpenAIVIPFastModeSettings); ok {
		return v
	}
	return nil
}

func (s *OpenAIGatewayService) openAIVIPFastModeSettings(ctx context.Context) *OpenAIVIPFastModeSettings {
	if settings := openAIVIPFastModeSettingsFromContext(ctx); settings != nil {
		return settings
	}
	if s == nil || s.settingService == nil {
		return DefaultOpenAIVIPFastModeSettings()
	}
	settings, err := s.settingService.GetOpenAIVIPFastModeSettings(ctx)
...
func openAIVIPFastModeHasTargets(settings *OpenAIVIPFastModeSettings) bool {
	return settings != nil && (len(settings.UserIDs) > 0 || len(settings.APIKeyIDs) > 0 || len(settings.GroupIDs) > 0)
}

func openAIVIPFastModeTargetMatches(settings *OpenAIVIPFastModeSettings, target OpenAIVIPFastModeTarget) bool {
	if settings == nil {
		return false
	}
	if containsInt64(settings.UserIDs, target.UserID) {
		return true
	}
	if containsInt64(settings.APIKeyIDs, target.APIKeyID) {
		return true
	}
	if target.GroupID != nil && containsInt64(settings.GroupIDs, *target.GroupID) {
		return true
	}
	return false
}

func (s *OpenAIGatewayService) shouldApplyOpenAIVIPFastMode(ctx context.Context, settings *OpenAIVIPFastModeSettings, model string) bool {
	if settings == nil || !settings.Enabled || !openAIVIPFastModeHasTargets(settings) {
		return false
	}
	target, ok := openAIVIPFastModeTargetFromContext(ctx)
	if !ok || !openAIVIPFastModeTargetMatches(settings, target) {
		return false
	}
	if len(settings.ModelWhitelist) > 0 && !matchModelWhitelist(model, settings.ModelWhitelist) {
		return false
	}
	return true
}

func (s *OpenAIGatewayService) applyOpenAIVIPFastModeToBody(ctx context.Context, account *Account, model string, body []byte) ([]byte, bool, error) {
	if len(body) == 0 {
		return body, false, nil
	}
	settings := s.openAIVIPFastModeSettings(ctx)
	if !s.shouldApplyOpenAIVIPFastMode(ctx, settings, model) {
		return body, false, nil
	}
	if settings.RespectClientServiceTier && gjson.GetBytes(body, "service_tier").Exists() {
		return body, false, nil
	}
	updated, err := sjson.SetBytes(body, "service_tier", OpenAIFastTierPriority)
	if err != nil {
		return body, false, fmt.Errorf("inject vip service_tier into body: %w", err)
	}
	_ = account
	return updated, true, nil
}

func (s *OpenAIGatewayService) applyOpenAIVIPFastModeToMap(ctx context.Context, account *Account, model string, body map[string]any) (bool, error) {
	if len(body) == 0 {
		return false, nil
	}
	settings := s.openAIVIPFastModeSettings(ctx)
	...
	if settings.RespectClientServiceTier {
		if _, exists := body["service_tier"]; exists {
			return false, nil
		}
	}
	body["service_tier"] = OpenAIFastTierPriority
	_ = account
	return true, nil
}
...
func (s *OpenAIGatewayService) applyOpenAIFastPolicyToWSResponseCreate(
	...
	var vipErr error
	if frame, _, vipErr = s.applyOpenAIVIPFastModeToBody(ctx, account, model, frame); vipErr != nil {
		return frame, nil, vipErr
	}
