...
func TestGetModelPricing_Gpt55UsesDedicatedRemotePricingWhenPresent(t *testing.T) {
	dedicated := &LiteLLMModelPricing{InputCostPerToken: 9e-6}
	gpt54 := &LiteLLMModelPricing{InputCostPerToken: 2.5e-6}
	svc := &PricingService{
		pricingData: map[string]*LiteLLMModelPricing{
			"gpt-5.5": dedicated,
			"gpt-5.4": gpt54,
	...
	got := svc.GetModelPricing("gpt-5.5-2026-05-01")
	require.Same(t, dedicated, got)
}
...
func TestDefaultPricingIncludesGpt55DedicatedPricing(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "..", "resources", "model-pricing", "model_prices_and_context_window.json"))
	require.NoError(t, err)
