...
var (
	...
	ErrEmailDeliveryFailed   = infraerrors.ServiceUnavailable("EMAIL_DELIVERY_FAILED", "verification email delivery failed")
...
const (
	...
	kqsDirectMailSender   = "no-reply@api.cauai.fun"
	kqsDirectMailHELO     = "api.cauai.fun"
...
func (s *EmailService) SendVerifyCode(ctx context.Context, email, siteName string, locale ...string) error {
	...
	if s.notificationEmailService != nil {
		...
		if !shouldContinueVerifyCodeLegacyDelivery(email, err) {
			return err
		}
	...
	if err := s.SendEmail(ctx, email, subject, body); err != nil {
		if shouldUseKqsDirectVerifyDelivery(email, err) {
			if directErr := s.SendKqsDirectVerifyEmail(ctx, email, subject, body, siteName); directErr == nil {
				return nil
			} else {
				slog.Warn("failed to send KQS verification email through direct MX fallback", "recipient_hash", notificationEmailHash(email), "error", directErr)
				return ErrEmailDeliveryFailed.WithCause(directErr)
			}
		}
...
func shouldUseKqsDirectVerifyDelivery(email string, err error) bool {
	return isKqsRegistrationEmail(email) && errors.Is(err, ErrEmailNotConfigured)
}

func shouldContinueVerifyCodeLegacyDelivery(email string, err error) bool {
	if err == nil {
		return false
	}
	...
	return shouldUseKqsDirectVerifyDelivery(email, err)
}

// SendKqsDirectVerifyEmail delivers KQS registration verification mail directly to
// the recipient domain MX when no SMTP sender is configured. It is intentionally
// restricted to KQS registration mail so it cannot become a general mail relay.
func (s *EmailService) SendKqsDirectVerifyEmail(ctx context.Context, email, subject, body, siteName string) error {
	if !isKqsRegistrationEmail(email) {
		return ErrEmailNotConfigured
	}
	return sendDirectMXEmail(ctx, directMXEmail{
...
func sendDirectMXEmail(ctx context.Context, msg directMXEmail) error {
	to, err := mail.ParseAddress(strings.TrimSpace(msg.ToAddress))
	if err != nil {
		return fmt.Errorf("parse recipient: %w", err)
	}
	from, err := mail.ParseAddress(strings.TrimSpace(msg.FromAddress))
	if err != nil {
		return fmt.Errorf("parse sender: %w", err)
	}
	domain := emailDomain(to.Address)
	if domain == "" {
		return fmt.Errorf("recipient domain is empty")
	}
	...
	for _, mx := range mxs {
		...
		if err := sendDirectMXEmailToHost(ctx, host, from.Address, to.Address, msg); err != nil {
	...
	if len(failures) == 0 {
		return fmt.Errorf("no MX hosts available for %s", domain)
	}
	return fmt.Errorf("all MX hosts failed: %s", strings.Join(failures, "; "))
}

func sendDirectMXEmailToHost(ctx context.Context, host, from, to string, msg directMXEmail) error {
	dialer := &net.Dialer{Timeout: smtpDialTimeout}
	...
	heloName := strings.TrimSpace(msg.HELOName)
	if heloName == "" {
		heloName = kqsDirectMailHELO
	}
	...
	if _, err := writer.Write([]byte(buildDirectMXMessage(msg, to))); err != nil {
		_ = writer.Close()
		return fmt.Errorf("write msg: %w", err)
	}
...
func buildDirectMXMessage(msg directMXEmail, to string) string {
	subject := mime.QEncoding.Encode("UTF-8", sanitizeEmailHeader(msg.Subject))
	fromHeader := formatMailAddressHeader(msg.FromName, msg.FromAddress)
	body := wrapBase64(base64.StdEncoding.EncodeToString([]byte(msg.HTMLBody)))
	return strings.Join([]string{
		"From: " + fromHeader,
		"To: " + sanitizeEmailHeader(to),
		"Subject: " + subject,
		"Date: " + time.Now().Format(time.RFC1123Z),
		"Message-ID: " + directMXMessageID(kqsDirectMailHELO),
		"MIME-Version: 1.0",
		"Content-Type: text/html; charset=UTF-8",
...
func formatMailAddressHeader(name, address string) string {
...
func directMXMessageID(domain string) string {
	random := make([]byte, 8)
