...
// ProvidePublicChannelStatusService creates PublicChannelStatusService.
func ProvidePublicChannelStatusService(
	planRepo ScheduledTestPlanRepository,
	resultRepo ScheduledTestResultRepository,
) *PublicChannelStatusService {
	return NewPublicChannelStatusService(planRepo, resultRepo)
}
...
func ProvideScheduledTestRunnerService(
	...
	svc := NewScheduledTestRunnerService(planRepo, scheduledSvc, publicStatusSvc, accountTestSvc, rateLimitSvc, cfg)
	svc.Start()
	return svc
}
...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
	...
	if err := svc.LoadURLAllowlistUpstreamHostsSetting(context.Background()); err != nil {
		logger.LegacyPrintf("service.setting", "Warning: load url allowlist upstream hosts setting failed: %v", err)
	}
...
var ProviderSet = wire.NewSet(
	...
	ProvidePublicChannelStatusService,
