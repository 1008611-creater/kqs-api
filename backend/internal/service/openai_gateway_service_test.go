...
func TestOpenAIStreamingClientDisconnectAbortsUpstream(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
	...
	_ = pr.Close()
	require.ErrorIs(t, err, ErrOpenAIClientDisconnected)
	require.NotNil(t, result)
...
func TestOpenAIStreamingClientCancelAbortsBeforeUpstreamToken(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
	...
	case <-time.After(time.Second):
		t.Fatal("stream did not abort after client context cancellation")
	}
}
