...
func TestBuildUpstreamModelsRequestsForAPIKeyAccounts(t *testing.T) {
	...
	anthropicBearerReq, err := svc.buildAnthropicUpstreamModelsRequest(ctx, &Account{
		...
		Type:     AccountTypeAPIKey,
		Credentials: map[string]any{
			"api_key":  "anthropic-bearer-token",
	...
	require.NoError(t, err)
	require.Equal(t, "https://beecode.cc/v1/models", anthropicBearerReq.URL.String())
	require.Equal(t, "Bearer anthropic-bearer-token", anthropicBearerReq.Header.Get("Authorization"))
