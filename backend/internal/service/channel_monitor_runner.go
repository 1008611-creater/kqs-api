...
var channelMonitorScheduleLocation = time.FixedZone("Asia/Shanghai", 8*60*60)

const (
	monitorPeakIntervalSeconds    = 120
	monitorOffPeakIntervalSeconds = 180
)
...
func (r *ChannelMonitorRunner) runScheduled(ctx context.Context, task *scheduledMonitor) {
	...
	for {
		wait := r.nextInterval(task, time.Now())
...
func (r *ChannelMonitorRunner) nextInterval(task *scheduledMonitor, now time.Time) time.Duration {
	if task == nil {
		return 0
	}
	if !usesDynamicPublicMonitorInterval(task.groupName) {
		return task.interval
	}
	if isPublicMonitorOffPeak(now) {
		return monitorOffPeakIntervalSeconds * time.Second
	}
	return monitorPeakIntervalSeconds * time.Second
}

func usesDynamicPublicMonitorInterval(groupName string) bool {
	return groupName == PublicChannelStatusDisplayName ||
		groupName == PublicChannelStatusPro4DisplayName ||
		groupName == PublicChannelStatusGPT56DisplayName
}

func isPublicMonitorOffPeak(now time.Time) bool {
	localHour := now.In(channelMonitorScheduleLocation).Hour()
	switch {
	case localHour >= 3 && localHour < 7:
		return true
	case localHour >= 11 && localHour < 13:
		return true
	case localHour >= 18 && localHour < 19:
		return true
	default:
		return false
	}
