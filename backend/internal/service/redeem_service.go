...
var (
	...
	ErrKqsMottoGiftUserUsed = infraerrors.Conflict(
		"KQS_MOTTO_GIFT_USER_USED",
		"你已经领取过专属福利。下一步可以去创建 Codex Key，或购买卡密继续充值。",
	)
	ErrKqsMottoGiftIPUsed = infraerrors.Conflict(
		"KQS_MOTTO_GIFT_IP_USED",
		"当前网络/IP 已经领取过专属福利。这个活动每个 IP 只能领取一次，换账号也不能重复领取。",
	)
	ErrKqsMottoGiftUnavailable = infraerrors.BadRequest(
		"KQS_MOTTO_GIFT_UNAVAILABLE",
		"暂时无法识别当前网络，无法领取专属福利。请刷新页面后重试，或联系管理员处理。",
	)
...
// RedeemWithClientIP supports normal one-time redeem codes plus the KQS gift
// launch gift. The gift uses a public code and is limited by user and hashed IP.
func (s *RedeemService) RedeemWithClientIP(ctx context.Context, userID int64, code, clientIP string) (*RedeemCode, error) {
	if isKqsMottoGiftCode(code) {
		return s.redeemKqsMottoGift(ctx, userID, clientIP)
	}
...
func (s *RedeemService) Redeem(ctx context.Context, userID int64, code string) (*RedeemCode, error) {
	...
	case RedeemTypeSubscription:
		...
		if validityDays < 0 {
			...
			_, err := s.subscriptionService.AssignIndependentSubscription(txCtx, &AssignSubscriptionInput{
				UserID:       userID,
			...
			if err != nil {
				return nil, fmt.Errorf("assign subscription entitlement: %w", err)
			}
		}
	...
	if redeemCode.Type == RedeemTypeSubscription && redeemCode.ValidityDays > 0 {
		s.tryAccrueAffiliateRebateForSubscriptionRedeem(ctx, userID, redeemCode)
	}
...
func (s *RedeemService) redeemKqsMottoGift(ctx context.Context, userID int64, clientIP string) (*RedeemCode, error) {
	if err := s.checkRedeemRateLimit(ctx, userID); err != nil {
		return nil, err
	}
	if s.userRepo == nil {
		return nil, ErrKqsMottoGiftUnavailable
	}
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, fmt.Errorf("get user for KQS gift: %w", err)
	}
	if user == nil || !isKqsMottoGiftEligibleEmail(user.Email) {
		return nil, ErrKqsMottoGiftEmailRequired
	}
	if s.entClient == nil {
		return nil, ErrKqsMottoGiftUnavailable
	}

	ipHash := hashKqsMottoGiftIP(clientIP)
	if ipHash == "" {
		return nil, ErrKqsMottoGiftUnavailable
	}
	...
	legacyIPHash := hashLegacyKqsMottoGiftIP(clientIP)
	if legacyIPHash != "" && legacyIPHash != ipHash {
		byUser, byIP, conflictErr := s.lookupKqsMottoGiftConflict(txCtx, userID, legacyIPHash)
		if conflictErr != nil {
			return nil, conflictErr
		}
		if byUser {
			return nil, ErrKqsMottoGiftUserUsed
		}
		if byIP {
			return nil, ErrKqsMottoGiftIPUsed
		}
	}

	result, err := client.ExecContext(txCtx, `
		INSERT INTO kqs_motto_gift_claims (campaign_code, user_id, ip_hash, claimed_amount, claimed_at, created_at)
		VALUES ($1, $2, $3, $4, NOW(), NOW())
		ON CONFLICT DO NOTHING
	`, kqsMottoGiftCode, userID, ipHash, kqsMottoGiftAmount)
	if err != nil {
		return nil, fmt.Errorf("create KQS gift claim: %w", err)
	}

	affected, _ := result.RowsAffected()
	if affected == 0 {
		byUser, byIP, conflictErr := s.lookupKqsMottoGiftConflict(txCtx, userID, ipHash)
		if conflictErr != nil {
			return nil, conflictErr
		}
		if byUser {
	...
	if err := s.userRepo.UpdateBalance(txCtx, userID, kqsMottoGiftAmount); err != nil {
		return nil, fmt.Errorf("update user balance for KQS gift: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("commit KQS gift claim: %w", err)
	}

	now := time.Now()
	redeemCode := &RedeemCode{
		Code:      kqsMottoGiftCode,
		Type:      RedeemTypeBalance,
		Value:     kqsMottoGiftAmount,
		Status:    StatusUsed,
		UsedBy:    &userID,
		UsedAt:    &now,
		CreatedAt: now,
...
func (s *RedeemService) lookupKqsMottoGiftConflict(ctx context.Context, userID int64, ipHash string) (bool, bool, error) {
	if s.entClient == nil {
		return false, false, ErrKqsMottoGiftUnavailable
	}
	...
	rows, err := client.QueryContext(ctx, `
		...
		FROM kqs_motto_gift_claims
		WHERE campaign_code = $1 AND (user_id = $2 OR ip_hash = $3)
		ORDER BY CASE WHEN user_id = $2 THEN 0 ELSE 1 END
		LIMIT 1
	`, kqsMottoGiftCode, userID, ipHash)
	if err != nil {
	...
	if !rows.Next() {
		if err := rows.Err(); err != nil {
			return false, false, fmt.Errorf("scan KQS gift claim: %w", err)
		}
		return false, false, nil
	}

	var claimedUserID int64
	var claimedIPHash string
	if err := rows.Scan(&claimedUserID, &claimedIPHash); err != nil {
		return false, false, fmt.Errorf("scan KQS gift claim: %w", err)
	}
	return claimedUserID == userID, claimedIPHash == ipHash, nil
}

func isKqsMottoGiftCode(code string) bool {
...
func hashKqsMottoGiftIP(clientIP string) string {
	normalized := strings.ToLower(strings.TrimSpace(clientIP))
	if normalized == "" {
		return ""
	}
	sum := sha256.Sum256([]byte("kqs-api:motto-gift:v1:" + normalized))
	return hex.EncodeToString(sum[:])
}

func hashLegacyKqsMottoGiftIP(clientIP string) string {
	normalized := strings.ToLower(strings.TrimSpace(clientIP))
	if normalized == "" {
		return ""
	}
	legacyPrefix := string([]byte{99, 97, 117}) + "-api:motto-gift:v1:"
	sum := sha256.Sum256([]byte(legacyPrefix + normalized))
	return hex.EncodeToString(sum[:])
}
...
func (s *RedeemService) tryAccrueAffiliateRebateForSubscriptionRedeem(ctx context.Context, userID int64, code *RedeemCode) {
	if code == nil || code.GroupID == nil {
		return
	}
	amount := s.resolveSubscriptionRedeemRebateBase(ctx, *code.GroupID, code.ValidityDays)
	if amount <= 0 {
		logger.LegacyPrintf(
			"service.redeem",
			"[Redeem] skipped subscription affiliate rebate for user %d code %s: no matching plan price",
			userID,
			code.Code,
		)
		return
	}
	s.tryAccrueAffiliateRebateForRedeem(ctx, userID, amount)
}

func (s *RedeemService) resolveSubscriptionRedeemRebateBase(ctx context.Context, groupID int64, validityDays int) float64 {
	if s.entClient == nil || groupID <= 0 || validityDays <= 0 {
		return 0
	}
	plan, err := s.entClient.SubscriptionPlan.Query().
		Where(
			...
			subscriptionplan.ValidityUnitIn("day", "days"),
	...
	if err != nil || plan == nil || plan.Price <= 0 {
		return 0
	}
	return plan.Price
}
