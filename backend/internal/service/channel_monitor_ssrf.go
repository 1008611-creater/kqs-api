...
// isAllowedInternalMonitorEndpoint limits the only loopback exception to the
// service's own HTTP listener inside the app container. This keeps normal SSRF
// protection intact while allowing the public status checker to avoid routing
// back out through Cloudflare from a domestic server.
func isAllowedInternalMonitorEndpoint(hostname, port string) bool {
	if port != "8080" {
		return false
	}
...
func safeDialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	if ip := net.ParseIP(host); ip != nil {
		if isPrivateIP(ip) && !isAllowedInternalMonitorEndpoint(host, port) {
			return nil, &net.AddrError{Err: "blocked by SSRF policy", Addr: address}
		}
		return monitorDialer.DialContext(ctx, network, address)
	}
	if isBlockedHostname(host) && !isAllowedInternalMonitorEndpoint(host, port) {
		return nil, &net.AddrError{Err: "blocked by SSRF policy", Addr: address}
	}
