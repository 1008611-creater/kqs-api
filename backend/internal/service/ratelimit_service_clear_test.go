...
func TestRateLimitService_RecoverAccountAfterSuccessfulTest_ClearsStreamDisconnectTempUnsched(t *testing.T) {
	...
	repo := &rateLimitClearRepoStub{
		getByIDAccount: &Account{
			...
			TempUnschedulableReason: "OpenAI stream disconnected before completion",
...
func TestRateLimitService_RecoverAccountAfterSuccessfulTest_StreamDisconnectOnlyClearsTempUnsched(t *testing.T) {
	until := time.Now().Add(2 * time.Hour)
	repo := &rateLimitClearRepoStub{
		getByIDAccount: &Account{
			ID:                      44,
			...
			Schedulable:             true,
			TempUnschedulableUntil:  &until,
			TempUnschedulableReason: "stream closed before response.completed",
			Extra:                   map[string]any{},
		},
	}
	cache := &tempUnschedCacheRecorder{}
	...
	require.Equal(t, 1, repo.clearModelRateLimitCalls)
	require.Equal(t, 1, repo.clearTempUnschedCalls)
	require.Equal(t, []int64{44}, cache.deletedIDs)
}
