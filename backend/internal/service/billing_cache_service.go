...
func (s *BillingCacheService) convertFromPortsData(data *SubscriptionCacheData) *subscriptionCacheData {
	return &subscriptionCacheData{
		...
		DailyRollover: data.DailyRollover,
...
func (s *BillingCacheService) getSubscriptionFromDB(ctx context.Context, userID, groupID int64) (*subscriptionCacheData, error) {
	...
	return &subscriptionCacheData{
		...
		DailyRollover: sub.DailyRolloverUSD,
...
func (s *BillingCacheService) checkSubscriptionEligibility(ctx context.Context, userID int64, group *Group, subscription *UserSubscription) error {
	...
	current := subscription.CurrentWindowSnapshot(group)

	// 检查限额（使用传入的Group限额配置）
	if limit := current.EffectiveDailyLimitUSD(group); limit != nil {
		available := *limit
		if current.DailyRolloverUSD > 0 {
			available += current.DailyRolloverUSD
		}
		if current.DailyUsageUSD >= available {
			return ErrDailyLimitExceeded
		}
	}

	if limit := current.EffectiveWeeklyLimitUSD(group); limit != nil && current.WeeklyUsageUSD >= *limit {
		return ErrWeeklyLimitExceeded
	}

	if limit := current.EffectiveMonthlyLimitUSD(group); limit != nil && current.MonthlyUsageUSD >= *limit {
