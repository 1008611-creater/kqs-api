...
func newOpenAIGatewayServiceWithVIPFastModeSettings(t *testing.T, settings *OpenAIVIPFastModeSettings) *OpenAIGatewayService {
	t.Helper()
	repo := &openAIFastPolicyRepoStub{values: map[string]string{}}
	rawFast, err := json.Marshal(DefaultOpenAIFastPolicySettings())
	require.NoError(t, err)
	repo.values[SettingKeyOpenAIFastPolicySettings] = string(rawFast)
	if settings != nil {
		raw, err := json.Marshal(settings)
		require.NoError(t, err)
		repo.values[SettingKeyOpenAIVIPFastModeSettings] = string(raw)
	}
	return &OpenAIGatewayService{
		settingService: NewSettingService(repo, &config.Config{}),
	}
...
func openAIVIPFastModeTestContext() context.Context {
	groupID := int64(3)
	return WithOpenAIVIPFastModeTarget(context.Background(), OpenAIVIPFastModeTarget{
		UserID:   11,
		APIKeyID: 22,
		GroupID:  &groupID,
	})
...
func TestGetOpenAIVIPFastModeSettings_DefaultsRespectClientServiceTierWhenMissing(t *testing.T) {
	repo := &openAIFastPolicyRepoStub{values: map[string]string{
		SettingKeyOpenAIVIPFastModeSettings: `{"enabled":true,"user_ids":[11,0,11],"api_key_ids":[22],"group_ids":[3],"model_whitelist":[" gpt-5.5 ",""]}`,
	}}
	svc := NewSettingService(repo, &config.Config{})

	got, err := svc.GetOpenAIVIPFastModeSettings(context.Background())
	require.NoError(t, err)
	require.True(t, got.RespectClientServiceTier)
	require.Equal(t, []int64{11}, got.UserIDs)
	require.Equal(t, []int64{22}, got.APIKeyIDs)
	require.Equal(t, []int64{3}, got.GroupIDs)
	require.Equal(t, []string{"gpt-5.5"}, got.ModelWhitelist)

	repo.values[SettingKeyOpenAIVIPFastModeSettings] = `{"enabled":true,"user_ids":[11],"respect_client_service_tier":false}`
	got, err = svc.GetOpenAIVIPFastModeSettings(context.Background())
	require.NoError(t, err)
	require.False(t, got.RespectClientServiceTier)
}

func TestApplyOpenAIVIPFastModeToBodyTargetsAndRespectClientTier(t *testing.T) {
	svc := newOpenAIGatewayServiceWithVIPFastModeSettings(t, &OpenAIVIPFastModeSettings{
		Enabled:                  true,
		UserIDs:                  []int64{11},
		RespectClientServiceTier: true,
	})
	account := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey}

	body := []byte(`{"model":"gpt-5.5","input":"hi"}`)
	updated, injected, err := svc.applyOpenAIVIPFastModeToBody(openAIVIPFastModeTestContext(), account, "gpt-5.5", body)
	require.NoError(t, err)
...
func TestApplyOpenAIVIPFastModeToBodyForceOverridesClientTier(t *testing.T) {
	svc := newOpenAIGatewayServiceWithVIPFastModeSettings(t, &OpenAIVIPFastModeSettings{
		Enabled:                  true,
		UserIDs:                  []int64{11},
		RespectClientServiceTier: false,
	})
	account := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey}

	updated, injected, err := svc.applyOpenAIVIPFastModeToBody(
		openAIVIPFastModeTestContext(),
		account,
		"gpt-5.5",
...
func TestApplyOpenAIVIPFastModeToBodyRequiresExplicitTargetMatchAndWhitelist(t *testing.T) {
	account := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey}
	body := []byte(`{"model":"gpt-5.5","input":"hi"}`)
	otherGroupID := int64(9)
	mismatchedCtx := WithOpenAIVIPFastModeTarget(context.Background(), OpenAIVIPFastModeTarget{
		UserID:   99,
		APIKeyID: 77,
	...
	tests := []struct {
		...
		{
			name: "enabled without targets never enables global priority",
			ctx:  openAIVIPFastModeTestContext(),
			settings: &OpenAIVIPFastModeSettings{
				Enabled:                  true,
				RespectClientServiceTier: true,
			},
		...
		{
			name: "non matching target does not inject priority",
			ctx:  mismatchedCtx,
			settings: &OpenAIVIPFastModeSettings{
				Enabled:                  true,
				UserIDs:                  []int64{11},
		...
		{
			name: "whitelist miss does not inject priority",
			ctx:  openAIVIPFastModeTestContext(),
			settings: &OpenAIVIPFastModeSettings{
				Enabled:                  true,
				UserIDs:                  []int64{11},
				ModelWhitelist:           []string{"gpt-4.1"},
				RespectClientServiceTier: true,
			},
			model: "gpt-5.5",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := newOpenAIGatewayServiceWithVIPFastModeSettings(t, tt.settings)

			updated, injected, err := svc.applyOpenAIVIPFastModeToBody(tt.ctx, account, tt.model, body)
			require.NoError(t, err)
			require.False(t, injected)
...
func TestApplyOpenAIVIPFastModeToMapMatchesAPIKeyOrGroup(t *testing.T) {
	account := &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey}
	...
	injected, err := svc.applyOpenAIVIPFastModeToMap(openAIVIPFastModeTestContext(), account, "gpt-5.5", body)
	require.NoError(t, err)
	require.True(t, injected)
	require.Equal(t, OpenAIFastTierPriority, body["service_tier"])
