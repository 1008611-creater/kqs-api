...
const (
	...
	defaultLoginAgreementMode    = "checkbox"
	defaultLoginAgreementDate    = "2026-05-25"
	defaultLegalTermsContent     = `# 服务条款

欢迎使用矿泉水API。本服务面向学习、科研、课程项目和个人团队协作场景，提供 OpenAI 兼容格式的 API 转接、余额兑换、Key 管理和用量查看能力。
...
本站按现状提供服务。因上游限制、网络波动、用户配置错误、Key 泄露、滥用行为或第三方平台问题造成的不可用、额度消耗或业务损失，本站在法律允许范围内不承担间接损失责任。`
	defaultLegalPrivacyContent = `# 隐私说明
...
用量、兑换、订单、风控和系统日志会按运营需要保留。后续如果接入更完整的数据清理策略，会在站内公告。`
	defaultLegalRefundContent = `# 退款与卡密规则
...
请提供账号邮箱、订单号、卡密码后四位、支付时间和错误截图。管理员会尽量在 24 小时内处理 Beta 期间的订单问题。`
	defaultLegalUsagePolicyContent = `# 可接受使用政策
	...
	return []LoginAgreementDocument{
		{
			ID:        "terms",
			Title:     "服务条款",
			ContentMD: defaultLegalTermsContent,
		},
		{
			ID:        "privacy",
			Title:     "隐私说明",
			ContentMD: defaultLegalPrivacyContent,
		},
		{
			ID:        "refund",
			Title:     "退款与卡密规则",
			ContentMD: defaultLegalRefundContent,
		},
		{
			ID:        "usage-policy",
			Title:     "可接受使用政策",
			ContentMD: defaultLegalUsagePolicyContent,
		},
	}
}
...
func (s *SettingService) LoadURLAllowlistUpstreamHostsSetting(ctx context.Context) error {
	if s == nil || s.cfg == nil || s.settingRepo == nil {
		return nil
	}
	raw, err := s.settingRepo.GetValue(ctx, SettingKeySecurityURLAllowlistUpstreamHosts)
	if err != nil {
		if errors.Is(err, ErrSettingNotFound) {
			s.cfg.SetURLAllowlistUpstreamHosts(s.cfg.URLAllowlistUpstreamHosts())
			return nil
		}
		return fmt.Errorf("get upstream host allowlist setting: %w", err)
	}
	hosts := normalizeUpstreamAllowlistHosts(parseStringListSetting(raw))
	if len(hosts) == 0 {
		return nil
	}
	s.cfg.SetURLAllowlistUpstreamHosts(hosts)
	return nil
}

func (s *SettingService) GetURLAllowlistUpstreamHosts(ctx context.Context) []string {
	if s == nil || s.cfg == nil {
		return nil
	}
	if s.settingRepo != nil {
		raw, err := s.settingRepo.GetValue(ctx, SettingKeySecurityURLAllowlistUpstreamHosts)
		if err == nil {
			if hosts := normalizeUpstreamAllowlistHosts(parseStringListSetting(raw)); len(hosts) > 0 {
				s.cfg.SetURLAllowlistUpstreamHosts(hosts)
				return hosts
			}
		}
	}
	return s.cfg.URLAllowlistUpstreamHosts()
}

func (s *SettingService) SetURLAllowlistUpstreamHosts(ctx context.Context, hosts []string) ([]string, error) {
	if s == nil || s.cfg == nil || s.settingRepo == nil {
		return nil, errors.New("setting service is not initialized")
	}
	normalized := normalizeUpstreamAllowlistHosts(hosts)
	if len(normalized) == 0 {
		return nil, errors.New("upstream host allowlist cannot be empty")
	}
	raw, err := json.Marshal(normalized)
	if err != nil {
		return nil, fmt.Errorf("marshal upstream host allowlist: %w", err)
	}
	if err := s.settingRepo.Set(ctx, SettingKeySecurityURLAllowlistUpstreamHosts, string(raw)); err != nil {
		return nil, err
	}
	s.cfg.SetURLAllowlistUpstreamHosts(normalized)
	return normalized, nil
}

func normalizeUpstreamAllowlistHosts(values []string) []string {
	if len(values) == 0 {
		return []string{}
	}
	...
	for _, value := range values {
		...
		parseTarget := raw
		if !strings.HasPrefix(parseTarget, "https://") && !strings.HasPrefix(parseTarget, "http://") {
...
// GetOpenAIVIPFastModeSettings 获取 OpenAI VIP 快速模式配置
func (s *SettingService) GetOpenAIVIPFastModeSettings(ctx context.Context) (*OpenAIVIPFastModeSettings, error) {
	value, err := s.settingRepo.GetValue(ctx, SettingKeyOpenAIVIPFastModeSettings)
	if err != nil {
		if errors.Is(err, ErrSettingNotFound) {
			return DefaultOpenAIVIPFastModeSettings(), nil
		}
		return nil, fmt.Errorf("get openai vip fast mode settings: %w", err)
	}
	if value == "" {
		return DefaultOpenAIVIPFastModeSettings(), nil
	}

	var settings OpenAIVIPFastModeSettings
	if err := json.Unmarshal([]byte(value), &settings); err != nil {
		slog.Warn("failed to unmarshal openai vip fast mode settings, falling back to defaults",
			"error", err,
			"key", SettingKeyOpenAIVIPFastModeSettings)
		return DefaultOpenAIVIPFastModeSettings(), nil
	}
	...
	if _, ok := raw["respect_client_service_tier"]; !ok {
		settings.RespectClientServiceTier = true
	}
	settings.UserIDs = normalizePositiveInt64List(settings.UserIDs)
	settings.APIKeyIDs = normalizePositiveInt64List(settings.APIKeyIDs)
	settings.GroupIDs = normalizePositiveInt64List(settings.GroupIDs)
	settings.ModelWhitelist = normalizeNonEmptyStringList(settings.ModelWhitelist)
	return &settings, nil
}

// SetOpenAIVIPFastModeSettings 设置 OpenAI VIP 快速模式配置
func (s *SettingService) SetOpenAIVIPFastModeSettings(ctx context.Context, settings *OpenAIVIPFastModeSettings) error {
	if settings == nil {
		return fmt.Errorf("settings cannot be nil")
	}
	settings.UserIDs = normalizePositiveInt64List(settings.UserIDs)
	...
	data, err := json.Marshal(settings)
	if err != nil {
		return fmt.Errorf("marshal openai vip fast mode settings: %w", err)
	}
	return s.settingRepo.Set(ctx, SettingKeyOpenAIVIPFastModeSettings, string(data))
}
