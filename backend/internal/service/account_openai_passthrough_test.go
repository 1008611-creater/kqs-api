...
func TestAccount_IsOpenAIRequestedModelAllowed(t *testing.T) {
	t.Run("透传账号不被模型映射误拦截", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeOAuth,
		...
		require.True(t, account.IsOpenAIRequestedModelAllowed("gpt-5.4-mini"))
	})

	t.Run("非透传账号仍按模型白名单限制", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeOAuth,
			Credentials: map[string]any{
		...
		require.True(t, account.IsOpenAIRequestedModelAllowed("gpt-5.5"))
		require.True(t, account.IsOpenAIRequestedModelAllowed("gpt-5-nano"))
		require.False(t, account.IsOpenAIRequestedModelAllowed("gpt-5.4-mini"))
	})

	t.Run("空模型名默认允许", func(t *testing.T) {
		account := &Account{
			Platform: PlatformOpenAI,
			Type:     AccountTypeOAuth,
		}
		require.True(t, account.IsOpenAIRequestedModelAllowed(""))
	})
}
