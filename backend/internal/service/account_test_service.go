...
func (s *AccountTestService) validateUpstreamBaseURL(raw string) (string, error) {
	...
	if !s.cfg.Security.URLAllowlist.Enabled {
		return urlvalidator.ValidateURLFormatWithOptions(raw, s.cfg.Security.URLAllowlist.AllowInsecureHTTP, urlvalidator.ValidationOptions{
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	payload := createOpenAITestPayload(testModelID, isOAuth, true)
	payloadBytes, _ := json.Marshal(payload)
	...
	if err := detectOpenAIUpstreamHTML(resp.Header, resp.Body, "OpenAI Responses API"); err != nil {
...
func (s *AccountTestService) testOpenAIChatCompletionsConnection(
	...
	if err := detectOpenAIUpstreamHTML(resp.Header, resp.Body, "Chat Completions API (/v1/chat/completions)"); err != nil {
		return s.sendErrorAndEnd(c, err.Error())
	}
...
// testOpenAILightweightResponsesConnection is used by background monitoring only.
// It keeps the OpenAI Responses probe minimal by omitting the large default
// instructions block, and acts as a fallback when /v1/chat/completions is not
// supported by the upstream.
func (s *AccountTestService) testOpenAILightweightResponsesConnection(
	c *gin.Context,
	account *Account,
	testModelID string,
	normalizedBaseURL string,
	...
	payload := createOpenAITestPayload(testModelID, false, false)
	payloadBytes, _ := json.Marshal(payload)

	s.sendEvent(c, TestEvent{Type: "test_start", Model: testModelID})
	s.sendEvent(c, TestEvent{Type: "status", Text: "正在通过 /v1/responses 轻量探活"})
	...
	if err := detectOpenAIUpstreamHTML(resp.Header, resp.Body, "API (/v1/responses)"); err != nil {
		return s.sendErrorAndEnd(c, err.Error())
	}
...
func createOpenAITestPayload(modelID string, isOAuth bool, includeInstructions bool) map[string]any {
	payload := map[string]any{
		"model": modelID,
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				...
				return s.sendErrorAndEnd(c, "Invalid Chat Completions response from /v1/chat/completions: upstream may be an HTML page or non-SSE endpoint")
			}
			if seenJSON {
				return s.sendErrorAndEnd(c, fmt.Sprintf("Chat Completions stream read error from /v1/chat/completions: %s", err.Error()))
			}
			return s.sendErrorAndEnd(c, fmt.Sprintf("Invalid Chat Completions response from /v1/chat/completions: upstream may be an HTML page or non-SSE endpoint (%s)", err.Error()))
		}
		...
		if isLikelyHTMLResponseLine(line) {
			return s.sendErrorAndEnd(c, "Invalid Chat Completions response from /v1/chat/completions: upstream returned HTML instead of SSE")
		}
...
func (s *AccountTestService) processOpenAIStream(c *gin.Context, body io.Reader) error {
	...
	for {
		...
		if err != nil {
			...
			return s.sendErrorAndEnd(c, fmt.Sprintf("Invalid OpenAI Responses response: upstream may be an HTML page or non-SSE endpoint (%s)", err.Error()))
		}
		...
		if isLikelyHTMLResponseLine(line) {
			return s.sendErrorAndEnd(c, "Invalid OpenAI Responses response: upstream returned HTML instead of SSE")
		}
		if !sseDataPrefix.MatchString(line) {
			continue
		}
...
func isLikelyHTMLResponseLine(line string) bool {
	lower := strings.ToLower(strings.TrimSpace(line))
	if lower == "" {
		return false
	}
	return strings.HasPrefix(lower, "<!doctype html") || strings.HasPrefix(lower, "<html") || strings.HasPrefix(lower, "<head") || strings.HasPrefix(lower, "<body")
}

func detectOpenAIUpstreamHTML(headers http.Header, body io.Reader, endpoint string) error {
	_ = body
...
func (s *AccountTestService) runBackgroundWithRecorder(ctx context.Context, runner func(*gin.Context) error) (*ScheduledTestResult, error) {
	startedAt := time.Now()
...
func (s *AccountTestService) RunTestBackground(ctx context.Context, accountID int64, modelID string) (*ScheduledTestResult, error) {
	return s.runBackgroundWithRecorder(ctx, func(ginCtx *gin.Context) error {
		return s.TestAccountConnection(ginCtx, accountID, modelID, "", AccountTestModeDefault)
	})
}

// RunMonitorBackground executes the lightweight probe used by the public status
// monitor. OpenAI API key accounts first try the cheaper /v1/chat/completions
// path with a minimal prompt, then fall back to a lightweight /v1/responses
// probe without the large default instructions block.
func (s *AccountTestService) RunMonitorBackground(ctx context.Context, accountID int64, modelID string) (*ScheduledTestResult, error) {
	if s.accountRepo == nil {
	...
	if err != nil {
		return &ScheduledTestResult{
			Status:       "failed",
			ErrorMessage: fmt.Sprintf("Invalid base URL: %s", err.Error()),
			StartedAt:    time.Now(),
			FinishedAt:   time.Now(),
		}, nil
	}

	chatResult, _ := s.runBackgroundWithRecorder(ctx, func(ginCtx *gin.Context) error {
		return s.testOpenAIChatCompletionsConnection(ginCtx, account, testModelID, "hi", normalizedBaseURL, authToken)
	})
	if chatResult != nil && chatResult.Status == "success" {
		return chatResult, nil
	}

	responsesResult, _ := s.runBackgroundWithRecorder(ctx, func(ginCtx *gin.Context) error {
		return s.testOpenAILightweightResponsesConnection(ginCtx, account, testModelID, normalizedBaseURL, authToken)
	})
	if responsesResult != nil && responsesResult.Status == "success" {
		return responsesResult, nil
	}
	if responsesResult != nil && chatResult != nil &&
		strings.TrimSpace(chatResult.ErrorMessage) != "" &&
		strings.TrimSpace(responsesResult.ErrorMessage) != "" {
		responsesResult.ErrorMessage = fmt.Sprintf(
			"chat/completions failed: %s; responses fallback failed: %s",
			chatResult.ErrorMessage,
			responsesResult.ErrorMessage,
		)
	}
