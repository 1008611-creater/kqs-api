...
func (s *OpsService) GetURLAllowlistSettings(ctx context.Context) (*OpsURLAllowlistSettings, error) {
	hosts := []string{}
	if s != nil && s.cfg != nil {
		hosts = normalizeUpstreamAllowlistHosts(s.cfg.URLAllowlistUpstreamHosts())
	}
	if s == nil || s.settingRepo == nil {
		return &OpsURLAllowlistSettings{UpstreamHosts: hosts}, nil
	}
	if ctx == nil {
		ctx = context.Background()
	}
	...
	dbHosts := normalizeUpstreamAllowlistHosts(parseStringListSetting(raw))
	if len(dbHosts) == 0 {
		return &OpsURLAllowlistSettings{UpstreamHosts: hosts}, nil
	}
	if s.cfg != nil {
		s.cfg.SetURLAllowlistUpstreamHosts(dbHosts)
	}
	return &OpsURLAllowlistSettings{UpstreamHosts: dbHosts}, nil
}

func (s *OpsService) UpdateURLAllowlistSettings(ctx context.Context, cfg *OpsURLAllowlistSettings) (*OpsURLAllowlistSettings, error) {
	if s == nil || s.settingRepo == nil {
		return nil, errors.New("setting repository not initialized")
	}
	if ctx == nil {
	...
	hosts := normalizeUpstreamAllowlistHosts(cfg.UpstreamHosts)
	if len(hosts) == 0 {
		return nil, errors.New("upstream_hosts cannot be empty")
	}
