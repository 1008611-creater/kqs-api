...
// AssignIndependentSubscription creates a standalone entitlement for purchase
// and redeem flows. It intentionally does not extend an existing same-group
// subscription, so buying two identical cards gives two overlapping cards
// instead of one longer subscription.
...
func (s *SubscriptionService) CheckAndResetWindows(ctx context.Context, sub *UserSubscription) error {
	...
	// 日窗口重置（24小时）
	if sub.NeedsDailyReset() {
		rolloverUSD, err := s.calculateDailyRolloverForReset(ctx, sub)
		if err != nil {
			return err
		}
		if err := s.userSubRepo.ResetDailyUsage(ctx, sub.ID, windowStart, rolloverUSD); err != nil {
			return err
		}
		sub.DailyWindowStart = &windowStart
		sub.DailyUsageUSD = 0
		sub.DailyRolloverUSD = rolloverUSD
...
func (s *SubscriptionService) calculateDailyRolloverForReset(ctx context.Context, sub *UserSubscription) (float64, error) {
	if sub == nil {
		return 0, nil
	}
	_ = ctx
	return sub.CalculateNextDailyRolloverUSD(sub.Group), nil
}
...
func (s *SubscriptionService) ValidateAndCheckLimits(sub *UserSubscription, group *Group) (needsMaintenance bool, err error) {
	...
	if sub.NeedsDailyReset() {
		sub.DailyRolloverUSD = sub.CalculateNextDailyRolloverUSD(group)
		sub.DailyUsageUSD = 0
...
func (s *SubscriptionService) calculateProgress(sub *UserSubscription, group *Group) *SubscriptionProgress {
	...
	// 日进度
	if limitPtr := sub.EffectiveDailyLimitUSD(group); limitPtr != nil && sub.DailyWindowStart != nil {
		limit := *limitPtr
		available := limit
		if availablePtr := sub.EffectiveDailyAvailableUSD(group); availablePtr != nil {
		...
		progress.Daily = &UsageWindowProgress{
			...
			RemainingUSD:    available - sub.DailyUsageUSD,
			WindowStart:     *sub.DailyWindowStart,
			ResetsAt:        resetsAt,
			ResetsInSeconds: int64(time.Until(resetsAt).Seconds()),
		}
		if available > 0 {
			progress.Daily.Percentage = (sub.DailyUsageUSD / available) * 100
		}
	...
	// 周进度
	if limitPtr := sub.EffectiveWeeklyLimitUSD(group); limitPtr != nil && sub.WeeklyWindowStart != nil {
		limit := *limitPtr
	...
	// 月进度
	if limitPtr := sub.EffectiveMonthlyLimitUSD(group); limitPtr != nil && sub.MonthlyWindowStart != nil {
		limit := *limitPtr
