...
func TestGatewayService_AnthropicAPIKeyPassthrough_BearerAuthHeader(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
	...
	req, err := svc.buildUpstreamRequestAnthropicAPIKeyPassthrough(context.Background(), c, account, []byte(`{}`), "upstream-token")
	require.NoError(t, err)
	...
	require.Empty(t, getHeaderRaw(req.Header, "x-goog-api-key"))
	require.Empty(t, getHeaderRaw(req.Header, "cookie"))

	countReq, err := svc.buildCountTokensRequestAnthropicAPIKeyPassthrough(context.Background(), c, account, []byte(`{}`), "upstream-token")
	require.NoError(t, err)
	require.Equal(t, "Bearer upstream-token", getHeaderRaw(countReq.Header, "authorization"))
