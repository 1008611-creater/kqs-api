...
// IsOpenAIRequestedModelAllowed returns whether an OpenAI account can accept the
// client-requested model before forwarding. Passthrough accounts only replace
// authentication, so model_mapping must not accidentally block user selection.
func (a *Account) IsOpenAIRequestedModelAllowed(requestedModel string) bool {
	if strings.TrimSpace(requestedModel) == "" {
		return true
	}
	if a != nil && a.IsOpenAI() && a.IsOpenAIPassthroughEnabled() {
		return true
	}
	return a != nil && a.IsModelSupported(requestedModel)
}
...
// GetMaxRequestBodyBytes returns the optional per-account request body ceiling.
// It is stored in MiB in Extra so admins can enter human-scale values such as
// "10" for small relay accounts.
func (a *Account) GetMaxRequestBodyBytes() int64 {
	if a == nil {
		return 0
	}
	mb := a.getExtraFloat64("max_request_body_mb")
	if mb <= 0 {
		return 0
	}
