...
func TestNextInterval_UsesOffPeakScheduleForPublicMonitors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "plus off peak 03:30",
			groupName: PublicChannelStatusDisplayName,
			at:        time.Date(2026, 6, 23, 3, 30, 0, 0, channelMonitorScheduleLocation),
			want:      180 * time.Second,
		},
		{
			name:      "pro off peak 11:15",
			groupName: PublicChannelStatusPro4DisplayName,
			at:        time.Date(2026, 6, 23, 11, 15, 0, 0, channelMonitorScheduleLocation),
			want:      180 * time.Second,
		},
		{
			name:      "gpt56 off peak 18:30",
			groupName: PublicChannelStatusGPT56DisplayName,
			at:        time.Date(2026, 6, 23, 18, 30, 0, 0, channelMonitorScheduleLocation),
			want:      180 * time.Second,
		},
		{
			name:      "gpt56 peak 08:00",
			groupName: PublicChannelStatusGPT56DisplayName,
			at:        time.Date(2026, 6, 23, 8, 0, 0, 0, channelMonitorScheduleLocation),
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := r.nextInterval(task, tc.at); got != tc.want {
				t.Fatalf("nextInterval() = %v, want %v", got, tc.want)
			}
		})
	}
}

func TestNextInterval_NonPublicMonitorsKeepConfiguredInterval(t *testing.T) {
	r := newRunnerForTest(&stubMonitorSvc{})
	task := &scheduledMonitor{
		id:        2,
		name:      "custom",
	...
	got := r.nextInterval(task, time.Date(2026, 6, 23, 3, 30, 0, 0, channelMonitorScheduleLocation))
	if got != 75*time.Second {
		t.Fatalf("nextInterval() = %v, want %v", got, 75*time.Second)
	}
}
