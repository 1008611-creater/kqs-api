...
var (
	...
	ErrTurnstileActionMismatch     = infraerrors.BadRequest("TURNSTILE_ACTION_MISMATCH", "turnstile action mismatch")
)
...
func (s *TurnstileService) VerifyToken(ctx context.Context, token string, remoteIP string) error {
	return s.VerifyTokenForAction(ctx, token, remoteIP, "")
}

// VerifyTokenForAction 验证 Turnstile token，并在指定 action 时强制匹配。
func (s *TurnstileService) VerifyTokenForAction(ctx context.Context, token string, remoteIP string, expectedAction string) error {
	// 检查是否启用 Turnstile
	...
	if expectedAction = strings.TrimSpace(expectedAction); expectedAction != "" {
		actualAction := strings.TrimSpace(result.Action)
		if actualAction != "" && actualAction != expectedAction {
			logger.LegacyPrintf("service.turnstile", "[Turnstile] Action mismatch: expected=%s got=%s", expectedAction, actualAction)
			return ErrTurnstileActionMismatch
		}
		if actualAction == "" {
			logger.LegacyPrintf("service.turnstile", "[Turnstile] Verification succeeded without action, expected=%s", expectedAction)
		}
	}
