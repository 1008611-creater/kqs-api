...
const registrationTurnstileAction = "register"
const kqsRegistrationEmailSuffix = "@cau.edu.cn"
...
func isKqsRegistrationEmail(email string) bool {
	return strings.HasSuffix(strings.ToLower(strings.TrimSpace(email)), kqsRegistrationEmailSuffix)
}
...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affiliateCode string) (string, *User, error) {
	...
	if s.settingService != nil && s.settingService.IsInvitationCodeEnabled(ctx) {
		...
		if s.redeemRepo == nil {
			logger.LegacyPrintf("service.auth", "%s", "[Auth] Invitation code enabled but redeem repository is not configured")
...
func (s *AuthService) SendVerifyCodeAsync(ctx context.Context, email string, locale ...string) (*SendVerifyCodeResult, error) {
	...
	// 异步发送。发码入口已在 handler 层强制 Turnstile，所有注册邮箱统一走验证码。
...
func (s *AuthService) VerifyTurnstileForRegister(ctx context.Context, token, remoteIP, email, verifyCode string) error {
	if s == nil || s.settingService == nil {
		logger.LegacyPrintf("service.auth", "%s", "[Auth] Registration Turnstile required but settings service is not configured")
		return ErrTurnstileNotConfigured
	}

	if !s.settingService.IsTurnstileEnabled(ctx) {
		logger.LegacyPrintf("service.auth", "%s", "[Auth] Registration Turnstile disabled, skip Turnstile check")
		return nil
	}

	if err := s.ensureRegistrationTurnstileConfigured(ctx); err != nil {
		return err
	}
	...
	return s.VerifyTurnstileForAction(ctx, token, remoteIP, registrationTurnstileAction)
}

func (s *AuthService) ensureRegistrationTurnstileConfigured(ctx context.Context) error {
	if s == nil || s.settingService == nil || s.turnstileService == nil {
		logger.LegacyPrintf("service.auth", "%s", "[Auth] Registration Turnstile required but service is not configured")
		return ErrTurnstileNotConfigured
	}
	if !s.settingService.IsTurnstileEnabled(ctx) || s.settingService.GetTurnstileSecretKey(ctx) == "" {
		logger.LegacyPrintf("service.auth", "%s", "[Auth] Registration Turnstile required but keys are not configured")
		return ErrTurnstileNotConfigured
	}
	return nil
}
...
func (s *AuthService) VerifyTurnstile(ctx context.Context, token string, remoteIP string) error {
	return s.VerifyTurnstileForAction(ctx, token, remoteIP, "")
}

// VerifyTurnstileForAction verifies a Turnstile token and optionally enforces a widget action.
func (s *AuthService) VerifyTurnstileForAction(ctx context.Context, token string, remoteIP string, expectedAction string) error {
	...
	return s.turnstileService.VerifyTokenForAction(ctx, token, remoteIP, expectedAction)
}
