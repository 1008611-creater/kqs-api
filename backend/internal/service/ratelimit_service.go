...
func (s *RateLimitService) RecoverAccountState(ctx context.Context, accountID int64, options AccountRecoveryOptions) (*SuccessfulTestRecoveryResult, error) {
	...
	if hasRecoverableRuntimeStateExceptTempUnsched(account) {
		if hasProtectedStreamDisconnectTempUnsched(account) && !options.ClearTempUnsched {
			if err := s.clearRecoverableRuntimeStatePreserveTempUnsched(ctx, accountID); err != nil {
				return nil, err
			}
	...
	} else if options.ClearTempUnsched && account.TempUnschedulableUntil != nil {
		if err := s.ClearTempUnschedulable(ctx, accountID); err != nil {
			return nil, err
		}
		result.ClearedRateLimit = true
	}
...
func (s *RateLimitService) clearRecoverableRuntimeStatePreserveTempUnsched(ctx context.Context, accountID int64) error {
	if err := s.accountRepo.ClearRateLimit(ctx, accountID); err != nil {
		return err
	}
	if err := s.accountRepo.ClearAntigravityQuotaScopes(ctx, accountID); err != nil {
...
func (s *RateLimitService) RecoverAccountAfterSuccessfulTest(ctx context.Context, accountID int64) (*SuccessfulTestRecoveryResult, error) {
	return s.RecoverAccountState(ctx, accountID, AccountRecoveryOptions{ClearTempUnsched: true})
}
...
func hasProtectedStreamDisconnectTempUnsched(account *Account) bool {
	if account == nil || account.TempUnschedulableUntil == nil {
		return false
	}
	if account.TempUnschedulableUntil.Before(time.Now().Add(-1 * time.Minute)) {
		return false
	}
	...
	return strings.Contains(reason, "openai stream disconnected before completion") ||
		strings.Contains(reason, "openai_stream_disconnect") ||
		strings.Contains(reason, "stream disconnected before completion") ||
		strings.Contains(reason, "stream closed before response.completed") ||
		strings.Contains(reason, "stream_timeout_temp_unschedulable")
}
