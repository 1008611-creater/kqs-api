...
type ScheduledTestRunnerService struct {
	...
	publicStatusSvc *PublicChannelStatusService
...
func NewScheduledTestRunnerService(
	...
	return &ScheduledTestRunnerService{
		...
		publicStatusSvc: publicStatusSvc,
...
func (s *ScheduledTestRunnerService) runScheduled() {
	...
	now := time.Now()
	if s.publicStatusSvc != nil {
		if created, err := s.publicStatusSvc.EnsurePlans(ctx); err != nil {
			logger.LegacyPrintf("service.scheduled_test_runner", "[ScheduledTestRunner] ensure public status plans error: %v", err)
		} else if created > 0 {
			logger.LegacyPrintf("service.scheduled_test_runner", "[ScheduledTestRunner] ensured %d public status plans", created)
		}
	}
...
func (s *ScheduledTestRunnerService) runOnePlan(ctx context.Context, plan *ScheduledTestPlan) {
	result, err := s.accountTestSvc.RunMonitorBackground(ctx, plan.AccountID, plan.ModelID)
	if err != nil {
		logger.LegacyPrintf("service.scheduled_test_runner", "[ScheduledTestRunner] plan=%d RunMonitorBackground error: %v", plan.ID, err)
		return
	}
