...
func (s *GatewayService) buildUpstreamRequestAnthropicAPIKeyPassthrough(
	...
	// 覆盖入站鉴权残留，并注入上游认证
	account.applyAnthropicAPIKeyAuthHeader(req.Header, token)
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, error) {
	...
	} else if account != nil && account.Platform == PlatformAnthropic && account.Type == AccountTypeAPIKey {
		account.applyAnthropicAPIKeyAuthHeader(req.Header, token)
	} else {
...
// ExtractUpstreamErrorMessageForClient returns a sanitized, compact message that is
// safe to expose to API clients when the upstream response body contains a useful
// diagnosis (for example a 502 HTML error page or a malformed JSON gateway reply).
func ExtractUpstreamErrorMessageForClient(body []byte) string {
	return extractUpstreamErrorMessageForClient(body)
}
...
func extractUpstreamErrorMessageForClient(body []byte) string {
	...
	if msg != "" {
		return sanitizeClientFacingUpstreamMessage(msg)
	}
...
func sanitizeClientFacingUpstreamMessage(msg string) string {
	...
	const maxClientUpstreamMessageBytes = 320
	if len(msg) <= maxClientUpstreamMessageBytes {
		return msg
	}
	const ellipsis = "...(truncated)"
	cutoff := maxClientUpstreamMessageBytes - len(ellipsis)
	if cutoff < 0 {
		cutoff = 0
	}
