...
func TestExtractUpstreamErrorMessageForClient_FallsBackToSanitizedHTMLSnippet(t *testing.T) {
	t.Parallel()
...
<html>
  <body>
    ...
    <p>Unexpected token '<', "<!doctype html>" is not valid JSON.</p>
    <p>gateway=127.0.0.1:11643 key=***</p>
  </body>
</html>`)

	msg := ExtractUpstreamErrorMessageForClient(body)
	require.Contains(t, msg, "502 Bad Gateway")
	require.Contains(t, msg, "not valid JSON")
	require.NotContains(t, msg, "127.0.0.1:11643")
	require.NotContains(t, msg, "key=***")
}
