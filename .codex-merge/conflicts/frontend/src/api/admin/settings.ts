...
export function appendAuthSourceDefaultsToUpdateRequest(
  ...
  for (const source of AUTH_SOURCE_TYPES) {
    const current = authSourceDefaults[source] ?? fallbackDefaults[source];
...
=======
  openai_vip_fast_mode_settings?: OpenAIVIPFastModeSettings;
...
/**
 * OpenAI VIP fast mode settings interface.
 * Matches backend dto.OpenAIVIPFastModeSettings.
 */
export interface OpenAIVIPFastModeSettings {
  enabled: boolean;
  user_ids?: number[];
  api_key_ids?: number[];
  group_ids?: number[];
  model_whitelist?: string[];
  respect_client_service_tier: boolean;
}
