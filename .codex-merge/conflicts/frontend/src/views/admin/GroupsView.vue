  ...
  <AppLayout>
    ...
    >
      ...
      >
        ...
        <!-- Codex GPT + 国产模型合集导入（仅 openai 平台） -->
        <div v-if="createForm.platform === 'openai'" class="border-t pt-4">
          <div class="mb-1.5 flex items-center gap-1">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ t("admin.groups.codexAllModels.title") }}
            </label>
            <div class="group relative inline-flex">
              <Icon
                name="questionCircle"
              ...
              >
                <div class="rounded-lg bg-gray-900 p-3 text-white shadow-lg dark:bg-gray-800">
                  <p class="text-xs leading-relaxed text-gray-300">
                    {{ t("admin.groups.codexAllModels.tooltip") }}
                  </p>
          ...
          <label class="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              :checked="createForm.supported_model_scopes.includes('codex_all_models')"
              @change="toggleCreateScope('codex_all_models')"
              class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700"
            />
            <span class="text-sm text-gray-700 dark:text-gray-300">
              {{ t("admin.groups.codexAllModels.enabled") }}
            </span>
          </label>
          <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
            {{ t("admin.groups.codexAllModels.hint") }}
          </p>
        </div>
    ...
    >
      ...
      >
        ...
        <!-- 快速选择账号（编辑时） -->
        <div
          class="rounded-xl border border-gray-200 bg-gray-50/80 p-4 dark:border-dark-600 dark:bg-dark-800/40"
        >
          <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                只显示同平台账号；保存后会把此分组加入或移出所选账号，保留账号原有其他分组。
              </p>
            </div>
            <div class="flex flex-wrap items-center gap-2 text-xs">
              <span
                class="rounded-full bg-white px-2.5 py-1 font-medium text-gray-700 ring-1 ring-gray-200 dark:bg-dark-700 dark:text-gray-200 dark:ring-dark-500"
              >
                已选 {{ editAccountBindingSelectedCount }} / {{ editAccountBindingAccounts.length }}
              </span>
              <span
                v-if="editAccountBindingAddedCount || editAccountBindingRemovedCount"
                class="rounded-full bg-primary-50 px-2.5 py-1 font-medium text-primary-700 ring-1 ring-primary-100 dark:bg-primary-900/20 dark:text-primary-300 dark:ring-primary-900/40"
              >
                +{{ editAccountBindingAddedCount }} / -{{ editAccountBindingRemovedCount }}
              </span>
            </div>
          </div>
          ...
          <div
            v-if="editForm.copy_accounts_from_group_ids.length > 0"
            class="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-900/50 dark:bg-amber-900/20 dark:text-amber-300"
          >
            已选择“从分组复制账号”，本次保存会以复制结果为准，下面的手动选择暂不同步。
          </div>
          ...
          <div class="mt-3 flex flex-col gap-2 lg:flex-row lg:items-center">
            <div class="relative flex-1">
              ...
              />
              <input
                v-model="editAccountBindingSearch"
                type="text"
                class="input pl-9 text-sm"
                placeholder="搜索账号名称、ID、类型或状态"
              />
            </div>
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :disabled="
                  editAccountBindingLoading ||
                  editForm.copy_accounts_from_group_ids.length > 0 ||
                  editAccountBindingFilteredAccounts.length === 0
                "
                @click="selectFilteredEditAccounts"
              >
                选中筛选结果
              </button>
              <button
                ...
                :disabled="
                  editAccountBindingLoading ||
                  editForm.copy_accounts_from_group_ids.length > 0 ||
                  editAccountBindingSelectedCount === 0
                "
                @click="clearEditAccountSelection"
              >
                清空选择
              </button>
              <button
                type="button"
                class="btn btn-secondary btn-sm"
                :disabled="editAccountBindingLoading || !editingGroup"
                @click="editingGroup && loadEditAccountBindings(editingGroup)"
              >
                <Icon
                  name="refresh"
                  size="sm"
                  :class="editAccountBindingLoading ? 'animate-spin' : ''"
                />
              </button>
            </div>
          </div>
          ...
          <div
            class="mt-3 max-h-72 overflow-y-auto rounded-lg border border-gray-200 bg-white dark:border-dark-600 dark:bg-dark-800"
          >
            <div
              v-if="editAccountBindingLoading"
              class="flex items-center justify-center gap-2 px-4 py-8 text-sm text-gray-500 dark:text-gray-400"
            >
              <Icon name="refresh" size="sm" class="animate-spin" />
              正在加载账号...
            </div>
            <div
              v-else-if="editAccountBindingError"
              class="px-4 py-6 text-sm text-red-600 dark:text-red-400"
            >
              {{ editAccountBindingError }}
            </div>
            <div
              v-else-if="editAccountBindingFilteredAccounts.length === 0"
            ...
            <template v-else>
              <label
                v-for="account in editAccountBindingFilteredAccounts"
                :key="account.id"
                :class="[
                  'flex cursor-pointer items-start gap-3 border-b border-gray-100 px-4 py-3 transition-colors last:border-b-0 dark:border-dark-700',
                  isEditAccountSelected(account.id)
              ...
              >
                <input
                  type="checkbox"
                  class="mt-1 h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-500 dark:bg-dark-700"
                  :checked="isEditAccountSelected(account.id)"
                  :disabled="editForm.copy_accounts_from_group_ids.length > 0"
                  @change="toggleEditAccountSelection(account.id)"
                />
                <div class="min-w-0 flex-1">
                  ...
                  <div class="mt-1 flex flex-wrap items-center gap-1.5 text-xs">
                    <span
                      class="rounded bg-gray-100 px-1.5 py-0.5 text-gray-600 dark:bg-dark-700 dark:text-gray-300"
                    >
                      {{ account.type }}
                    </span>
                    <span
                    ...
                    <span class="text-gray-400">
                      {{ account.group_ids?.length || 0 }} 个分组
                    </span>
                  </div>
                </div>
                <span
                  v-if="
                    editAccountBindingInitialIds.has(account.id) &&
                    !isEditAccountSelected(account.id)
                  "
                  class="mt-1 rounded-full bg-red-50 px-2 py-0.5 text-xs font-medium text-red-600 dark:bg-red-900/20 dark:text-red-300"
                >
                  将移除
                </span>
                <span
                  v-else-if="
                    ...
                    isEditAccountSelected(account.id)
                  "
                  class="mt-1 rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-300"
                >
                  将加入
                </span>
              </label>
        ...
        <div v-if="editForm.platform === 'openai'" class="border-t pt-4">
          ...
          <label class="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              :checked="editForm.supported_model_scopes.includes('codex_all_models')"
              @change="toggleEditScope('codex_all_models')"
              class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-600 dark:bg-dark-700"
            />
            <span class="text-sm text-gray-700 dark:text-gray-300">
              {{ t("admin.groups.codexAllModels.enabled") }}
...
const editAccountBindingAccounts = ref<Account[]>([]);
const editAccountBindingInitialIds = ref<Set<number>>(new Set());
const editAccountBindingSelectedIds = ref<Set<number>>(new Set());
const editAccountBindingSearch = ref("");
const editAccountBindingLoading = ref(false);
const editAccountBindingLoaded = ref(false);
const editAccountBindingError = ref("");
let editAccountBindingLoadSeq = 0;

const normalizeSearchText = (value: unknown) =>
  String(value ?? "").trim().toLowerCase();

const accountHasGroup = (account: Account, groupId: number) =>
  Array.isArray(account.group_ids) && account.group_ids.includes(groupId);

const editAccountBindingSelectedCount = computed(
  () => editAccountBindingSelectedIds.value.size,
);

const editAccountBindingAddedCount = computed(() => {
  let count = 0;
  for (const id of editAccountBindingSelectedIds.value) {
    if (!editAccountBindingInitialIds.value.has(id)) count += 1;
  }
  return count;
});

const editAccountBindingRemovedCount = computed(() => {
  let count = 0;
  for (const id of editAccountBindingInitialIds.value) {
    if (!editAccountBindingSelectedIds.value.has(id)) count += 1;
  }
  return count;
});

const editAccountBindingFilteredAccounts = computed(() => {
  const keyword = normalizeSearchText(editAccountBindingSearch.value);
  if (!keyword) return editAccountBindingAccounts.value;
  return editAccountBindingAccounts.value.filter((account) => {
    const haystack = [
      account.id,
      account.name,
      account.type,
      account.status,
      account.schedulable ? "可调度 schedulable" : "不调度 disabled",
...
const resetEditAccountBindings = () => {
  editAccountBindingLoadSeq += 1;
  editAccountBindingAccounts.value = [];
  editAccountBindingInitialIds.value = new Set();
  editAccountBindingSelectedIds.value = new Set();
  editAccountBindingSearch.value = "";
  editAccountBindingLoading.value = false;
  editAccountBindingLoaded.value = false;
  editAccountBindingError.value = "";
};

const isEditAccountSelected = (accountId: number) =>
  editAccountBindingSelectedIds.value.has(accountId);

const toggleEditAccountSelection = (accountId: number) => {
  if (editForm.copy_accounts_from_group_ids.length > 0) return;
  const next = new Set(editAccountBindingSelectedIds.value);
  if (next.has(accountId)) {
    next.delete(accountId);
  } else {
    next.add(accountId);
  }
  editAccountBindingSelectedIds.value = next;
};

const selectFilteredEditAccounts = () => {
  if (editForm.copy_accounts_from_group_ids.length > 0) return;
  const next = new Set(editAccountBindingSelectedIds.value);
  for (const account of editAccountBindingFilteredAccounts.value) {
    next.add(account.id);
  }
  editAccountBindingSelectedIds.value = next;
};

const clearEditAccountSelection = () => {
  if (editForm.copy_accounts_from_group_ids.length > 0) return;
  editAccountBindingSelectedIds.value = new Set();
};

const loadEditAccountBindings = async (group: AdminGroup) => {
  const seq = ++editAccountBindingLoadSeq;
  editAccountBindingLoading.value = true;
  editAccountBindingLoaded.value = false;
  editAccountBindingError.value = "";
  editAccountBindingSearch.value = "";
  ...
  try {
    ...
    if (seq !== editAccountBindingLoadSeq) return;

    const selected = new Set(
      accounts
        .filter((account) => accountHasGroup(account, group.id))
        .map((account) => account.id),
    );
    editAccountBindingAccounts.value = accounts;
    editAccountBindingInitialIds.value = new Set(selected);
    editAccountBindingSelectedIds.value = selected;
    editAccountBindingLoaded.value = true;
  } catch (error) {
    if (seq !== editAccountBindingLoadSeq) return;
    editAccountBindingError.value = "账号列表加载失败，请刷新后重试。";
    console.error("Error loading group account bindings:", error);
  } finally {
    if (seq === editAccountBindingLoadSeq) {
      editAccountBindingLoading.value = false;
    }
  }
};

const uniqueSortedIDs = (ids: Iterable<number>) =>
  Array.from(new Set(ids)).sort((a, b) => a - b);

const syncEditAccountBindings = async (groupId: number) => {
  if (
    !editAccountBindingLoaded.value ||
    editForm.copy_accounts_from_group_ids.length > 0
  ) {
    return { added: 0, removed: 0 };
  }

  const initial = editAccountBindingInitialIds.value;
  const selected = editAccountBindingSelectedIds.value;
  const changedAccounts = editAccountBindingAccounts.value.filter(
    (account) => initial.has(account.id) !== selected.has(account.id),
  );
  ...
  for (const account of changedAccounts) {
    const nextGroupIDs = new Set(account.group_ids ?? []);
    const shouldBind = selected.has(account.id);
    if (shouldBind) {
    ...
    try {
      const updated = await adminAPI.accounts.update(account.id, {
        group_ids: uniqueSortedIDs(nextGroupIDs),
        confirm_mixed_channel_risk: true,
      });
      account.group_ids = updated.group_ids ?? uniqueSortedIDs(nextGroupIDs);
      if (shouldBind) {
        added += 1;
      } else {
        removed += 1;
  ...
  if (errors.length > 0) {
    throw new Error(
      `账号绑定部分失败：${errors.slice(0, 3).join("；")}${
        errors.length > 3 ? `；另有 ${errors.length - 3} 个失败` : ""
      }`,
    );
  }
...
const handleEdit = async (group: AdminGroup) => {
  ...
  void loadEditAccountBindings(group);
};
...
=======
    editAccountBindingLoading.value &&
    editForm.copy_accounts_from_group_ids.length === 0
  ) {
    appStore.showWarning("账号列表还在加载，请稍等几秒再保存。");
>>>>>>> 1157e4cfe6271a81c9dc96df84af6c6a3b22f831
    return;
  }
  ...
  const shouldSyncAccountBindings =
    editAccountBindingLoaded.value &&
    editForm.copy_accounts_from_group_ids.length === 0;
  try {
...
=======
    ...
    const syncResult = shouldSyncAccountBindings
      ? await syncEditAccountBindings(groupId)
      : { added: 0, removed: 0 };
    if (syncResult.added || syncResult.removed) {
      appStore.showSuccess(
        `分组已更新，账号绑定新增 ${syncResult.added} 个，移除 ${syncResult.removed} 个`,
      );
    } else if (editForm.copy_accounts_from_group_ids.length > 0) {
      appStore.showSuccess("分组已更新，账号绑定已按复制分组规则处理。");
    } else {
      appStore.showSuccess(t("admin.groups.groupUpdated"));
    }
>>>>>>> 1157e4cfe6271a81c9dc96df84af6c6a3b22f831
