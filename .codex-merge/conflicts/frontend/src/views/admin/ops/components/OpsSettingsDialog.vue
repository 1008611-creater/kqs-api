...
import { opsAPI, type OpsURLAllowlistSettings } from '@/api/admin/ops'
...
// 上游地址白名单
const urlAllowlistSettings = ref<OpsURLAllowlistSettings | null>(null)
const urlAllowlistInput = ref('')
// 指标阈值配置
...
=======
    urlAllowlistSettings.value = urlAllowlist
    urlAllowlistInput.value = normalizeURLAllowlistInput(urlAllowlist.upstream_hosts)
...
const validation = computed(() => {
  ...
  if (urlAllowlistSettings.value && parseURLAllowlistInput(urlAllowlistInput.value).length === 0) {
    errors.push('上游地址白名单不能为空')
  }

  return { valid: errors.length === 0, errors }
})

function normalizeURLAllowlistInput(hosts: string[]): string {
  return hosts
...
function parseURLAllowlistInput(raw: string): string[] {
...
async function saveAllSettings() {
  ...
  try {
    ...
    await Promise.all([
      ...
      urlAllowlistSettings.value
        ? opsAPI.updateURLAllowlistSettings({
            upstream_hosts: parseURLAllowlistInput(urlAllowlistInput.value)
          })
        : Promise.resolve(),
...
<template>
  <BaseDialog :show="show" :title="t('admin.ops.settings.title')" width="extra-wide" @close="emit('close')">
    ...
    <div v-else-if="runtimeSettings && emailConfig && advancedSettings && urlAllowlistSettings" class="space-y-6">
      <!-- 验证错误 -->
      ...
      <!-- 上游地址白名单 -->
      <div class="rounded-2xl bg-gray-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-3 text-sm font-semibold text-gray-900 dark:text-white">上游地址白名单</h4>
        <p class="mb-3 text-xs text-gray-500 dark:text-gray-400">
          每行一个域名或 URL。这里控制后台新增/同步上游账号时允许使用的上游地址。
        </p>
        <textarea
          v-model="urlAllowlistInput"
          class="input min-h-[132px] font-mono text-xs"
          spellcheck="false"
          placeholder="api.example.com&#10;https://example.com"
        ></textarea>
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
          保存后立即生效；为空会被后端拒绝，避免误清空安全白名单。
        </p>
      </div>
