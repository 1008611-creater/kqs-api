<template>
  ...
  >
    ...
    >
      ...
      <div class="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4 dark:border-emerald-900/50 dark:bg-emerald-950/20">
        <div class="mb-3 flex flex-wrap items-start justify-between gap-3">
          ...
          <div class="flex flex-wrap gap-2">
            <button
              ...
              :key="preset.key"
              type="button"
              class="rounded-full border border-emerald-200 bg-white px-3 py-1.5 text-xs font-semibold text-emerald-800 transition hover:-translate-y-0.5 hover:border-emerald-400 hover:shadow-sm dark:border-emerald-800 dark:bg-dark-700 dark:text-emerald-200"
              @click="applySupplyPreset(preset.key)"
            >
              {{ preset.label }}
            </button>
          </div>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
          <div>
            <label class="input-label">{{ t('admin.accounts.supplyProfile.priority') }}</label>
            <p class="input-hint">{{ t('admin.accounts.supplyProfile.priorityHint') }}</p>
          </div>
          <div>
            <label class="input-label">{{ t('admin.accounts.supplyProfile.weight') }}</label>
            <p class="input-hint">{{ t('admin.accounts.supplyProfile.weightHint') }}</p>
          </div>
          <div>
...
type SupplyPresetKey = 'lowCostSmall' | 'balanced' | 'fallback'
const supplyPresets = computed<{ key: SupplyPresetKey; label: string }[]>(() => [
...
const applySupplyPreset = (preset: SupplyPresetKey) => {
  switch (preset) {
    case 'lowCostSmall':
      form.priority = 1
      form.load_factor = 1
...
=======
  ...
  mixedScheduling.value = extra?.mixed_scheduling === true
  allowOverages.value = extra?.allow_overages === true
  const maxBodyValue = Number(extra?.max_request_body_mb ?? 0)
  maxRequestBodyMB.value = Number.isFinite(maxBodyValue) && maxBodyValue > 0
    ? Math.trunc(maxBodyValue)
    : null
...
const applyCommonSupplyExtraToPayload = (updatePayload: Record<string, unknown>) => {
  ...
  if (Number.isFinite(maxBody) && maxBody > 0) {
    newExtra.max_request_body_mb = Math.trunc(maxBody)
  } else {
    delete newExtra.max_request_body_mb
  }
...
=======
    applyCommonSupplyExtraToPayload(updatePayload)
>>>>>>> 1157e4cfe6271a81c9dc96df84af6c6a3b22f831
