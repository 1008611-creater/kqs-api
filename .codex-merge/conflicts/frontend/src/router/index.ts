...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    ...(USER_SUBSCRIPTIONS_VISIBLE
      ? { component: () => import('@/views/user/SubscriptionShopView.vue') }
      : { redirect: '/dashboard' }),
    meta: {
      requiresAuth: true,
      requiresAdmin: false,
  ...
  },
  {
    ...
    component: () => import('@/views/user/BalancePurchaseView.vue'),
    meta: {
      requiresAuth: true,
      requiresAdmin: false,
      title: 'Buy Balance',
...
=======
    path: '/admin/channel-speed',
    name: 'AdminChannelSpeed',
    component: () => import('@/views/admin/ChannelSpeedView.vue'),
    meta: {
      requiresAuth: true,
      requiresAdmin: true,
      title: 'Channel Speed',
      titleKey: 'nav.channelSpeed'
...
const RETIRED_INTERFACE_PREFIXES = [
  '/available-channels',
  '/admin/channels',
]
...
function isRetiredInterfacePath(path: string): boolean {
  return RETIRED_INTERFACE_PREFIXES.some((prefix) => path === prefix || path.startsWith(prefix + '/'))
}
...
=======
  ...
  if (to.name === 'CustomPage') {
    ...
    if (menuItem?.label) {
      const siteName = appStore.siteName || '矿泉水API'
...
=======
  if (isRetiredInterfacePath(to.path)) {
    next(to.path.startsWith('/admin') || authStore.isAdmin ? '/admin/dashboard' : '/dashboard')
    return
  }
