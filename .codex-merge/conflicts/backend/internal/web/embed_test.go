...
func requireEmbeddedJSAssetPath(t *testing.T) string {
	t.Helper()

	var assetPath string
	err := fs.WalkDir(frontendFS, "dist/assets", func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
...
func TestFrontendServer_ServeIndexHTML(t *testing.T) {
	...
	t.Run("override_index_uses_html_handler_not_static_file", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"site_name": "OverrideSite"},
		}
		...
		require.NoError(t, os.WriteFile(
			filepath.Join(overrideDir, "index.html"),
			[]byte(`<!doctype html><html><head><title>Old</title></head><body><div id="app"></div></body></html>`),
			0o600,
		))
		...
		assert.Contains(t, w.Body.String(), "<title>OverrideSite - AI API Gateway</title>")
	})

	t.Run("skips_source_app_config_injection_for_cloudflare_worker_origin_request", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"site_name": "EdgeSite"},
		}
		...
		w := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(w)
		c.Request = httptest.NewRequest(http.MethodGet, "https://origin-api.cauai.fun/", nil)
		c.Request.Host = "origin-api.cauai.fun"
		c.Request.Header.Set("X-Forwarded-Host", "api.cauai.fun")
		...
		assert.Contains(t, w.Body.String(), "<title>EdgeSite - AI API Gateway</title>")
	})
...
=======
	...
	t.Run("serves_hashed_assets_with_long_cache", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
		...
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, requireEmbeddedJSAssetPath(t), nil)
		router.ServeHTTP(w, req)
	...
	t.Run("returns_404_for_missing_static_assets", func(t *testing.T) {
		...
		for _, path := range []string{"/assets/missing.css", "/missing.js", "/favicon-missing.ico"} {
			t.Run(path, func(t *testing.T) {
				w := httptest.NewRecorder()
				req := httptest.NewRequest(http.MethodGet, path, nil)
				router.ServeHTTP(w, req)
	...
	t.Run("serves_index_for_spa_routes_with_dots", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
		...
		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/users/jane.doe", nil)
		router.ServeHTTP(w, req)
