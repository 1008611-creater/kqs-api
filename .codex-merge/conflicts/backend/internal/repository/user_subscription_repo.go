...
func (r *userSubscriptionRepository) Create(ctx context.Context, sub *service.UserSubscription) error {
	...
	builder := client.UserSubscription.Create().
		...
		SetDailyRolloverUsd(sub.DailyRolloverUSD).
		...
		SetQuotaBonusMultiplier(normalizeQuotaBonusMultiplier(sub.QuotaBonusMultiplier)).
		SetQuotaBonusSource(sub.QuotaBonusSource).
...
func (r *userSubscriptionRepository) GetActiveByUserIDAndGroupID(ctx context.Context, userID, groupID int64) (*service.UserSubscription, error) {
	...
	models, err := client.UserSubscription.Query().
		...
		WithGroup().
		Order(dbent.Asc(usersubscription.FieldExpiresAt), dbent.Asc(usersubscription.FieldCreatedAt), dbent.Asc(usersubscription.FieldID)).
		All(ctx)
	if err != nil {
		return nil, err
	}
	...
	subs := userSubscriptionEntitiesToService(models)
	for i := range subs {
		sub := &subs[i]
		if sub.HasBillableQuota(sub.Group) {
			return sub, nil
		}
	}

	// All active entitlements are currently exhausted. Return the earliest
	// expiring one so callers surface a quota error rather than "not found".
	return &subs[0], nil
}
...
=======
func (r *userSubscriptionRepository) ResetDailyUsage(ctx context.Context, id int64, newWindowStart time.Time, rolloverUSD float64) error {
	if rolloverUSD < 0 {
		rolloverUSD = 0
	}
>>>>>>> 1157e4cfe6271a81c9dc96df84af6c6a3b22f831
	...
	n, err := query.
		SetDailyUsageUsd(0).
		SetDailyRolloverUsd(rolloverUSD).
...
=======
		...
		MonthlyWindowStart:      m.MonthlyWindowStart,
		DailyUsageUSD:           m.DailyUsageUsd,
		DailyRolloverUSD:        m.DailyRolloverUsd,
		WeeklyUsageUSD:          m.WeeklyUsageUsd,
		MonthlyUsageUSD:         m.MonthlyUsageUsd,
		...
		QuotaBonusMultiplier:    m.QuotaBonusMultiplier,
		QuotaBonusSource:        derefString(m.QuotaBonusSource),
		AssignedBy:              m.AssignedBy,
		AssignedAt:              m.AssignedAt,
		Notes:                   derefString(m.Notes),
		CreatedAt:               m.CreatedAt,
...
func normalizeQuotaBonusMultiplier(v float64) float64 {
	if v <= 0 {
		return 1
	}
	return v
