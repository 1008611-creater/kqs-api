...
func (h *AccountHandler) importCodexSessions(ctx context.Context, req CodexSessionImportRequest, entries []codexImportEntry) (CodexSessionImportResult, error) {
	...
	for _, entry := range entries {
		item, err := h.normalizeCodexImportEntry(ctx, req, entry)
		if err != nil {
			message := codexImportSafeMessage(err)
			result.Failed++
		...
		if expiryErr != nil {
			message := codexImportSafeMessage(expiryErr)
			result.Failed++
			result.Items = append(result.Items, CodexSessionImportItem{
				Index:   entry.Index,
				Name:    accountName,
		...
		if existing != nil && updateExisting {
			...
			if updateErr != nil {
				message := codexImportSafeMessage(updateErr)
				result.Failed++
				result.Items = append(result.Items, CodexSessionImportItem{
					Index:   entry.Index,
					Name:    accountName,
		...
		if createErr != nil {
			message := codexImportSafeMessage(createErr)
			result.Failed++
			result.Items = append(result.Items, CodexSessionImportItem{
				Index:   entry.Index,
				Name:    accountName,
...
func (h *AccountHandler) normalizeCodexImportEntry(ctx context.Context, req CodexSessionImportRequest, entry codexImportEntry) (*codexImportAccount, error) {
	prepared, err := h.prepareCodexImportEntry(ctx, req, entry)
	if err != nil {
		return nil, err
	}
	return normalizeCodexImportEntry(prepared)
}

func (h *AccountHandler) prepareCodexImportEntry(ctx context.Context, req CodexSessionImportRequest, entry codexImportEntry) (codexImportEntry, error) {
	refreshToken, clientID, hasAccessToken := codexImportRefreshOnlyInput(entry.Value)
	if refreshToken == "" || hasAccessToken {
		return entry, nil
	}
	if h.openaiOAuthService == nil {
		return entry, errors.New("仅 refresh_token 导入需要 OpenAI OAuth 刷新服务")
	}

	proxyURL, err := h.codexImportProxyURL(ctx, req.ProxyID)
	if err != nil {
		return entry, err
	}
	...
	if err != nil {
		return entry, fmt.Errorf("refresh_token 换取 accessToken 失败: %w", err)
	}
	return codexImportEntry{
		Index: entry.Index,
		Value: buildCodexImportValueFromRefresh(entry.Value, refreshToken, clientID, tokenInfo),
	}, nil
}

func (h *AccountHandler) codexImportProxyURL(ctx context.Context, proxyID *int64) (string, error) {
	if proxyID == nil {
		return "", nil
	}
	proxy, err := h.adminService.GetProxy(ctx, *proxyID)
	if err != nil {
		return "", fmt.Errorf("代理不存在或不可用: %w", err)
	}
	if proxy == nil {
		return "", nil
	}
...
func codexImportSafeMessage(err error) string {
	if err == nil {
		return ""
	}
...
func codexImportRefreshOnlyInput(value any) (refreshToken string, clientID string, hasAccessToken bool) {
	...
	case map[string]any:
		accessToken := firstCodexString(raw,
			...
			[]string{"tokens", "OPENAI_API_KEY"},
			[]string{"credentials", "access_token"},
			[]string{"credentials", "accessToken"},
			[]string{"credentials", "OPENAI_API_KEY"},
			[]string{"access_token"},
			[]string{"accessToken"},
		...
		clientID := firstCodexString(raw,
			[]string{"tokens", "client_id"},
			[]string{"tokens", "clientId"},
			[]string{"client_id"},
			[]string{"clientId"},
		)
		return refreshToken, clientID, strings.TrimSpace(accessToken) != ""
	default:
		return "", "", false
	}
}

func buildCodexImportValueFromRefresh(original any, originalRefreshToken, clientID string, tokenInfo *service.OpenAITokenInfo) map[string]any {
	out := map[string]any{}
	if raw, ok := original.(map[string]any); ok {
	...
	if refreshToken == "" {
		refreshToken = strings.TrimSpace(originalRefreshToken)
	}
	effectiveClientID := strings.TrimSpace(tokenInfo.ClientID)
	if effectiveClientID == "" {
		effectiveClientID = strings.TrimSpace(clientID)
	}

	out["accessToken"] = strings.TrimSpace(tokenInfo.AccessToken)
	out["access_token"] = strings.TrimSpace(tokenInfo.AccessToken)
	out["refresh_token"] = refreshToken
	if strings.TrimSpace(tokenInfo.IDToken) != "" {
		out["id_token"] = strings.TrimSpace(tokenInfo.IDToken)
	}
	if effectiveClientID != "" {
		out["client_id"] = effectiveClientID
	}
	if tokenInfo.ExpiresAt > 0 {
		out["expires_at"] = tokenInfo.ExpiresAt
	}
	if strings.TrimSpace(tokenInfo.Email) != "" {
		out["email"] = strings.TrimSpace(tokenInfo.Email)
	}
	if strings.TrimSpace(tokenInfo.ChatGPTAccountID) != "" {
		out["account_id"] = strings.TrimSpace(tokenInfo.ChatGPTAccountID)
	}
	if strings.TrimSpace(tokenInfo.ChatGPTUserID) != "" {
		out["user_id"] = strings.TrimSpace(tokenInfo.ChatGPTUserID)
	}
	if strings.TrimSpace(tokenInfo.OrganizationID) != "" {
		out["organization_id"] = strings.TrimSpace(tokenInfo.OrganizationID)
	}
	if strings.TrimSpace(tokenInfo.PlanType) != "" {
		out["plan_type"] = strings.TrimSpace(tokenInfo.PlanType)
	}
	...
	if strings.TrimSpace(tokenInfo.IDToken) != "" {
		tokens["id_token"] = strings.TrimSpace(tokenInfo.IDToken)
	}
	if effectiveClientID != "" {
		tokens["client_id"] = effectiveClientID
	}
	if tokenInfo.ExpiresAt > 0 {
		tokens["expires_at"] = tokenInfo.ExpiresAt
	}
	out["tokens"] = mergeCodexImportMap(codexMapValue(out["tokens"]), tokens)
	return out
}
...
func flattenCodexImportValues(values []any) []any {
	...
	appendValue = func(value any) {
		...
		if m, ok := value.(map[string]any); ok {
			if nested, ok := codexImportNestedValues(m); ok {
				appendValue(nested)
				return
			}
		}
...
func codexImportNestedValues(value map[string]any) (any, bool) {
	for _, key := range []string{"accounts", "items", "records", "sessions"} {
		if nested, ok := value[key]; ok && codexImportLooksLikeCollection(nested) {
			return nested, true
		}
	}
	if data, ok := value["data"]; ok {
		if codexImportLooksLikeCollection(data) {
			return data, true
		}
		if nested, ok := data.(map[string]any); ok {
			return codexImportNestedValues(nested)
		}
	}
	return nil, false
}

func codexImportLooksLikeCollection(value any) bool {
	switch raw := value.(type) {
	case []any:
		return true
	case map[string]any:
		...
		return hasAccounts || hasItems || hasRecords || hasSessions
	default:
		return false
	}
}
...
func normalizeCodexImportEntry(entry codexImportEntry) (*codexImportAccount, error) {
	...
	case map[string]any:
		...
		item.Email = firstCodexString(raw, []string{"email"}, []string{"user", "email"}, []string{"credentials", "email"})
	...
	if item.RefreshToken != "" {
		...
		item.Credentials["client_id"] = clientID
	}
	if item.IDToken != "" {
