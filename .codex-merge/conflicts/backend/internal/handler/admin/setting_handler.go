...
func (h *SettingHandler) GetSettings(c *gin.Context) {
	...
	if vipFastMode, err := h.settingService.GetOpenAIVIPFastModeSettings(c.Request.Context()); err != nil {
		slog.Error("openai_vip_fast_mode_settings_get_failed", "error", err)
	} else if vipFastMode != nil {
		payload.OpenAIVIPFastModeSettings = openaiVIPFastModeSettingsToDTO(vipFastMode)
	}
...
func openaiVIPFastModeSettingsToDTO(s *service.OpenAIVIPFastModeSettings) *dto.OpenAIVIPFastModeSettings {
	if s == nil {
		return nil
	}
	return &dto.OpenAIVIPFastModeSettings{
		Enabled:                  s.Enabled,
		UserIDs:                  append([]int64(nil), s.UserIDs...),
		APIKeyIDs:                append([]int64(nil), s.APIKeyIDs...),
		GroupIDs:                 append([]int64(nil), s.GroupIDs...),
		ModelWhitelist:           append([]string(nil), s.ModelWhitelist...),
		RespectClientServiceTier: s.RespectClientServiceTier,
	}
}

func openaiVIPFastModeSettingsFromDTO(s *dto.OpenAIVIPFastModeSettings) *service.OpenAIVIPFastModeSettings {
	if s == nil {
		return nil
	}
	return &service.OpenAIVIPFastModeSettings{
		Enabled:                  s.Enabled,
		UserIDs:                  append([]int64(nil), s.UserIDs...),
		APIKeyIDs:                append([]int64(nil), s.APIKeyIDs...),
		GroupIDs:                 append([]int64(nil), s.GroupIDs...),
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.OpenAIVIPFastModeSettings != nil {
		if err := h.settingService.SetOpenAIVIPFastModeSettings(c.Request.Context(), openaiVIPFastModeSettingsFromDTO(req.OpenAIVIPFastModeSettings)); err != nil {
			response.BadRequest(c, err.Error())
			return
		}
	}
