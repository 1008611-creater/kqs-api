...
func TestAssignIndependentSubscriptionCreatesSeparateEntitlements(t *testing.T) {
	groupRepo := &subscriptionGroupRepoStub{
		group: &Group{ID: 1, SubscriptionType: SubscriptionTypeSubscription},
	}
	subRepo := newSubscriptionUserSubRepoStub()
	...
	first, err := svc.AssignIndependentSubscription(context.Background(), &AssignSubscriptionInput{
		UserID:       1001,
		GroupID:      1,
		ValidityDays: 7,
	...
	second, err := svc.AssignIndependentSubscription(context.Background(), &AssignSubscriptionInput{
		UserID:       1001,
		GroupID:      1,
		ValidityDays: 7,
	...
	require.NotEqual(t, first.ID, second.ID, "duplicate purchases should create separate subscription rows")
	require.Equal(t, 2, subRepo.createCalls)
	require.LessOrEqual(t, second.ExpiresAt.Sub(second.StartsAt), 7*24*time.Hour+time.Second)
	require.Less(t, second.ExpiresAt, first.ExpiresAt.Add(24*time.Hour), "second purchase must not extend from first expiry")
}
