...
=======
	...
	SettingKeyAffiliateRebateDurationDays      = "affiliate_rebate_duration_days"      // 返利有效期（天，0=永久）
	SettingKeyAffiliateRebatePerInviteeCap     = "affiliate_rebate_per_invitee_cap"    // 单人返利上限（0=无上限）
	SettingKeyOpsProxyTrafficPools             = "ops_proxy_traffic_pools"             // 管理员代理/机场订阅流量池配置（JSON）
	SettingKeyRiskControlEnabled               = "risk_control_enabled"                // 是否启用风控中心入口与审计链路
	SettingKeyContentModerationConfig          = "content_moderation_config"           // 内容审计配置（JSON）
	SettingKeyLoginAgreementEnabled            = "login_agreement_enabled"             // 登录前是否要求同意条款
	SettingKeyLoginAgreementMode               = "login_agreement_mode"                // 条款确认展示模式：modal / checkbox
...
>>>>>>> 1157e4cfe6271a81c9dc96df84af6c6a3b22f831
	...
	// SettingKeySecurityURLAllowlistUpstreamHosts stores the runtime-editable upstream host allowlist.
	SettingKeySecurityURLAllowlistUpstreamHosts = "security_url_allowlist_upstream_hosts"
	...
	// SettingKeyOpenAIVIPFastModeSettings stores JSON config for server-side
	// priority service_tier injection for selected users, API keys, or groups.
	SettingKeyOpenAIVIPFastModeSettings = "openai_vip_fast_mode_settings"
