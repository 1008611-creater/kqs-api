...
=======
func TestForwardAsChatCompletions_VIPFastModePropagatesFinalServiceTier(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_vip_tier"}},
	...
	result, err := svc.ForwardAsChatCompletions(openAIVIPFastModeTestContext(), c, account, body, "", "gpt-5.1")
	require.NoError(t, err)
	require.NotNil(t, result)
